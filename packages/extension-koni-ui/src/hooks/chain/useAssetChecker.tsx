...
export default function useAssetChecker () {
  ...
  useEffect(() => {
    if (enablingAsset && assetSettingMap[enablingAsset]?.visible) {
      ...
      const message = t('ui.hook.chain.useAssetChecker.nameIsTurnedOn', { replace: { name: assetInfo?.symbol } });
  ...
  return useCallback((assetSlug: string) => {
    ...
    if ((assetInfo && !assetSetting) || !assetSetting.visible) {
      const message = t('ui.hook.chain.useAssetChecker.confirmTurnOnNetwork', {
        replace: {
      ...
      const _onEnabled = () => {
        updateAssetSetting({
          ...
          notify({ message: t('ui.hook.chain.useAssetChecker.nameIsTurningOn', { replace: { name: assetInfo?.symbol } }), duration: 1.5 });
        }).catch(console.error);
      };
      ...
      const btn = <Button
        ...
        size={'xs'}
      >
        {t('ui.hook.chain.useAssetChecker.turnItOn')}
      </Button>;
      ...
      const message = t('ui.hook.chain.useAssetChecker.chainIsDisconnected', { replace: { name: chainInfo?.name } });
