...
const usePreCheckAction = (address?: string, blockAllAccount = true, message?: string): ((onClick: VoidFunction, action: ExtrinsicType) => VoidFunction) => {
  ...
  const getAccountTypeTitle = useCallback((signMode: AccountSignMode): string => {
    switch (signMode) {
      ...
      case AccountSignMode.GENERIC_LEDGER:
        return t('ui.hook.account.usePreCheckAction.ledgerAccount');
      case AccountSignMode.ALL_ACCOUNT:
        return t('ui.hook.account.usePreCheckAction.allAccount');
      case AccountSignMode.PASSWORD:
        return t('ui.hook.account.usePreCheckAction.normalAccount');
      case AccountSignMode.QR:
        return t('ui.hook.account.usePreCheckAction.qrSignerAccount');
      case AccountSignMode.READ_ONLY:
        return t('ui.hook.account.usePreCheckAction.watchOnlyAccount');
      case AccountSignMode.UNKNOWN:
      default:
        return t('ui.hook.account.usePreCheckAction.unknownAccount');
    }
  }, [t]);
  ...
  return useCallback((onClick: VoidFunction, action: ExtrinsicType) => {
    return () => {
      if (!account) {
        notify({
          message: t('ui.hook.account.usePreCheckAction.accountNotExists'),
          type: 'info',
          duration: 1.5
        });
      } else {
        ...
        let defaultMessage = detectTranslate('ui.hook.account.usePreCheckAction.featureNotAvailableForAccountType');

        if (ALL_STAKING_ACTIONS.includes(action)) {
          defaultMessage = detectTranslate('ui.hook.account.usePreCheckAction.earningNotSupportedForAccountType');
        }
        ...
        if (!account.transactionActions.includes(action) || (mode === AccountSignMode.QR && account.chainType === 'ethereum' && isProductionMode)) {
          ...
          switch (mode) {
            ...
            case AccountSignMode.QR:
              accountTitle = t('ui.hook.account.usePreCheckAction.evmQrSignerAccount');
              break;

            case AccountSignMode.LEGACY_LEDGER:
            case AccountSignMode.GENERIC_LEDGER:
              if (account.chainType === AccountChainType.ETHEREUM) {
                accountTitle = t('ui.hook.account.usePreCheckAction.ledgerEvmAccount');
              } else if (account.chainType === AccountChainType.SUBSTRATE) {
                accountTitle = t('ui.hook.account.usePreCheckAction.ledgerSubstrateAccount');
              }
