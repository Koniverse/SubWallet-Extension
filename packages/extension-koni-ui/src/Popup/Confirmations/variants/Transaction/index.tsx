...
const getTransactionComponent = (extrinsicType: ExtrinsicType): typeof BaseTransactionConfirmation => {
  switch (extrinsicType) {
    ...
    case ExtrinsicType.ADD_PROXY:
      return AddProxyTransactionConfirmation;
    case ExtrinsicType.REMOVE_PROXY:
      return RemoveProxyTransactionConfirmation;
      ...
      <div className={CN(className, 'confirmation-content')}>
        ...
        {!!transaction.estimateFee?.tooHigh && (
          <AlertBox
            ...
            description={t('ui.TRANSACTION.Confirmations.highGasFeeWarning', { replace: { networkName: network?.name } })}
            title={t('ui.TRANSACTION.Confirmations.payAttentionExclamation')}
            type='warning'
          />
        )}
      </div>
