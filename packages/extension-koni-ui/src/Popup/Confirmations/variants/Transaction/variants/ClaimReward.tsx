  ...
  useEffect(() => {
    ...
    if (isRewardLteFee) {
      ...
      openAlert({
        ...
        okButton: {
          text: t('ui.Confirmations.Transaction.ClaimReward.iUnderstand'),
          onClick: closeAlert,
          icon: CheckCircle
        }
      });
  ...
  return (
    <>
      ...
      >
        {
          data.unclaimedReward && <MetaInfo.Number
            decimals={decimals}
            label={t('ui.Confirmations.Transaction.ClaimReward.availableRewards')}
            suffix={symbol}
        ...
        <MetaInfo.Number
          decimals={decimals}
          label={t('ui.Confirmations.Transaction.ClaimReward.estimatedFee')}
          suffix={symbol}
      ...
      <span className={CN('text-light-4')}>
        {
          data.bondReward
            ? t('ui.Confirmations.Transaction.ClaimReward.rewardsStakedBack')
            : t('ui.Confirmations.Transaction.ClaimReward.rewardsAddedToTransferable')
        }
      </span>
    </>
  );
