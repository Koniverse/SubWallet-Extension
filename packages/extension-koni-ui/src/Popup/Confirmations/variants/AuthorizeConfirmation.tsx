...
function Component ({ className, request }: Props) {
  ...
  const noAvailableTitle = useMemo(() => {
    if (accountAuthTypes && accountAuthTypes.length === 1) {
      switch (accountAuthTypes[0]) {
        case 'substrate':
          return t('ui.Confirmations.Authorize.noAvailableSubstrateAccount');
        case 'evm':
          return t('ui.Confirmations.Authorize.noAvailableEvmAccount');
        case 'ton':
          return t('ui.Confirmations.Authorize.noAvailableTonAccount');
        case 'cardano':
          return t('ui.Confirmations.Authorize.noAvailableCardanoAccount');
        case 'bitcoin':
          return t('ui.Confirmations.Authorize.noAvailableBitcoinAccount');
      }
    }

    return t('ui.Confirmations.Authorize.noAvailableAccount');
  }, [accountAuthTypes, t]);
  ...
  const noAvailableDescription = useMemo(() => {
    if (accountAuthTypes && accountAuthTypes.length === 1) {
      switch (accountAuthTypes[0]) {
        case 'substrate':
          return t('ui.Confirmations.Authorize.noSubstrateAccountToConnect');
        case 'evm':
          return t('ui.Confirmations.Authorize.noEvmAccountToConnect');
        case 'cardano':
          return t('ui.Confirmations.Authorize.noCardanoAccountToConnect');
        case 'bitcoin':
          return t('ui.Confirmations.Authorize.noBitcoinAccountToConnect');
      }
    }

    return t('ui.Confirmations.Authorize.noAccountToConnect');
  }, [accountAuthTypes, t]);
        ...
        >
          {
            visibleAccountProxies.length === 0
              ? noAvailableTitle
              : t('ui.Confirmations.Authorize.chooseAccountsToConnect')
          }
        </div>
        {
        ...
        <div className='description'>
          {
            visibleAccountProxies.length === 0
              ...
              : t('ui.Confirmations.Authorize.trustSiteBeforeConnecting')
          }
        </div>
      </div>
          ...
          (
            <>
              ...
              <Button
                ...
                schema={'secondary'}
              >
                {t('ui.Confirmations.Authorize.cancel')}
              </Button>
              <Button
                ...
                onClick={onConfirm}
              >
                {t('ui.Confirmations.Authorize.connect')}
              </Button>
            </>
          )
        }
          ...
          visibleAccountProxies.length === 0 &&
            (
              <>
                ...
                >
                  {t('ui.Confirmations.Authorize.createOne')}
                </Button>
              </>
            )
        }
