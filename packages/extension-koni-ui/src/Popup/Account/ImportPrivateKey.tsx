  ...
  const accountNameValidator = useCallback(async (validate: RuleObject, value: string) => {
    if (value) {
      try {
        ...
        if (!isValid) {
          return Promise.reject(t('ui.Account.ImportPrivateKey.accountNameInUse'));
        }
      } catch (e) {
        return Promise.reject(t('ui.Account.ImportPrivateKey.accountNameInvalid'));
      }
    }
    ...
    if (amount) {
      if (privateKey?.trim()) {
        ...
        if (privateKeyChanged) {
          setValidateState({
            status: 'error',
            message: t('ui.Account.ImportPrivateKey.privateKeyRequired')
          });
        }
      }
    }
  ...
  return (
    <PageWrapper className={CN(className)}>
      <Layout.WithSubHeaderOnly
        onBack={onBack}
        rightFooterButton={{
          children: validating ? t('ui.Account.ImportPrivateKey.validating') : t('ui.Account.ImportPrivateKey.importAccount'),
          icon: FooterIcon,
          onClick: form.submit,
        ...
        title={t<string>('ui.Account.ImportPrivateKey.importFromPrivateKey')}
      >
        <div className='container'>
          <div className='description'>
            {t('ui.Account.ImportPrivateKey.enterPrivateKeyToImport')}
          </div>
          <Form
          ...
          >
            ...
            >
              <PrivateKeyInput
                ...
                hideText={!show}
                label={t('ui.Account.ImportPrivateKey.privateKey')}
                placeholder={t('ui.Account.ImportPrivateKey.enterPrivateKey')}
            ...
            <Form.Item
              ...
              name={'name'}
              rules={[{
                message: t('ui.Account.ImportPrivateKey.accountNameRequired'),
            ...
            >
              <Input
                ...
                disabled={loading}
                label={t('ui.Account.ImportPrivateKey.accountName')}
                placeholder={t('ui.Account.ImportPrivateKey.enterAccountName')}
              />
            </Form.Item>
            <div className='button-container'>
              <Button
                ...
                type='ghost'
              >
                {show ? t('ui.Account.ImportPrivateKey.hidePrivateKey') : t('ui.Account.ImportPrivateKey.showPrivateKey')}
              </Button>
            </div>
          </Form>
        </div>
