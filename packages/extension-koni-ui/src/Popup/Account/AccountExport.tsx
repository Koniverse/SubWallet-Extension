...
const Component: React.FC<Props> = (props: Props) => {
  ...
  const titleMap = useMemo((): Record<ExportType, string> => ({
    [ExportType.JSON_FILE]: t('ui.Account.Export.successful'),
    [ExportType.QR_CODE]: t('ui.Account.Export.yourQrCode'),
    [ExportType.PRIVATE_KEY]: t('ui.Account.Export.yourPrivateKey'),
    [ExportType.SEED_PHRASE]: t('ui.Account.Export.yourSeedPhrase')
  }), [t]);
      ...
      promise
        ...
        .catch((e: Error) => {
          ...
          if (message === 'Unable to decode using the supplied passphrase') {
            message = t('ui.Account.Export.wrongPassword');
          }
    ...
    return [
      {
        ...
        hidden: false,
        icon: Leaf,
        label: t('ui.Account.Export.exportSeedPhrase'),
      ...
      },
      {
        ...
        hidden: false,
        icon: FileJs,
        label: t('ui.Account.Export.exportJsonFile'),
      ...
      },
      {
        ...
        hidden: false,
        icon: Wallet,
        label: t('ui.Account.Export.exportPrivateKey'),
      ...
      },
      {
        ...
        hidden: false,
        icon: QrCode,
        label: t('ui.Account.Export.exportQrCode'),
  ...
  return (
    <PageWrapper className={CN(className)}>
      <Layout.WithSubHeaderOnly
        ...
        onBack={onBack}
        rightFooterButton={{
          children: firstStep ? t('ui.Account.Export.confirm') : t('ui.Account.Export.finish'),
        ...
        title={
          firstStep
            ? t('ui.Account.Export.exportAccount')
            : !exportSingle
              ? t('ui.Account.Export.exportSuccessful')
      ...
      >
        <div className='body-container'>
          <div className={CN('notice', { 'mb-large': !firstStep })}>
            <AlertBox
              description={t('ui.Account.Export.privateKeyWarning')}
              title={t('ui.Account.Export.warningNeverDiscloseKey')}
              type='warning'
            />
          </div>
          {
            firstStep && (
              ...
              >
                <Form.Item
                  ...
                  rules={[
                    {
                      message: t('ui.Account.Export.passwordIsRequired'),
                      required: true
                    }
                  ]}
                ...
                >
                  <Input.Password
                    disabled={loading}
                    placeholder={t('ui.Account.Export.typeYourSubWalletPassword')}
                    suffix={<span />}
                    type='password'
                  />
                </Form.Item>
              ...
              >
                  ...
                  exportTypes.includes(ExportType.PRIVATE_KEY) && (
                    <div className='result-content'>
                      ...
                      <Button
                        ...
                        type='ghost'
                      >
                        {t('ui.Account.Export.copyToClipboard')}
                      </Button>
                    </div>
                  )
                }
                ...
                }
                {
                  exportTypes.includes(ExportType.JSON_FILE) && jsonData && (
                    <div className='result-content'>
                      <div className='result-title'>{t('ui.Account.Export.yourJsonFile')}</div>
                      {
                        exportSingle && (
                          <>
                            ...
                            <div className='json-done-tile'>
                              {t('ui.Account.Export.success')}
                            </div>
                            <div className='json-done-description'>
                              {t('ui.Account.Export.successfullyExportedJson')}
                            </div>
                          </>
                        )
                      }
