...
const Component: React.FC<Props> = ({ className }: Props) => {
  ...
  const listItem = useMemo<ListItem[]>(() => {
    ...
    if (exitedAccount.length) {
      exitedAccount.unshift({
        id: 'existed_accounts',
        groupLabel: t('ui.Account.RestoreJson.existedAccount')
      });
    ...
    uploadFile.originFileObj?.arrayBuffer()
      .then((bytes) => {
        ...
        if (!isValidJsonFile(json)) {
          throw new Error(t('ui.Account.RestoreJson.invalidJsonFile'));
        }
  ...
  const openExitedAccountNameWarningModal = useCallback(() => {
    alertModal.open({
      ...
      content:
        <div>
          {t('ui.Account.RestoreJson.duplicateAccountNameNumberedInfo')}
          <a
            className={'__modal-user-guide'}
            href={CHANGE_ACCOUNT_NAME}
            target='__blank'
          >
            {t('ui.Account.RestoreJson.thisGuide')}
          </a>
        </div>,
      title: t('ui.Account.RestoreJson.duplicateAccountName'),
      okButton: {
        text: t('ui.Account.RestoreJson.iUnderstand'),
        icon: CheckCircle,
        iconWeight: 'fill',
        onClick: () => {
          alertModal.close();
  ...
  const onChangePassword: ChangeEventHandler<HTMLInputElement> = useCallback((event) => {
    ...
    if (!value) {
      setPasswordValidateState({
        message: t('ui.Account.RestoreJson.passwordIsRequired'),
        status: 'error'
      });
    } else {
      setPasswordValidateState({});
  ...
  const footerContent = useMemo(() => {
    if (stepState === StepState.UPLOAD_JSON_FILE) {
      return t('ui.Account.RestoreJson.unlockFile');
    } else {
      if (accountProxiesSelected.length === 0) {
        return t('ui.Account.RestoreJson.importAccount');
      } else if (accountProxiesSelected.length === 1) {
        return t('ui.Account.RestoreJson.importOneAccount');
      } else {
        return t('ui.Account.RestoreJson.importSelectedAccounts', { replace: { accountProxiesSelected: accountProxiesSelected.length } });
      }
    }
  ...
  const titlePage = useMemo(() => {
    if (stepState === StepState.UPLOAD_JSON_FILE) {
      return t('ui.Account.RestoreJson.importFromJsonFile');
    } else {
      return t('ui.Account.RestoreJson.importAccount');
    }
  }, [stepState, t]);
        ...
        <div className={CN('container')}>
...

          {
            stepState === StepState.SELECT_ACCOUNT_IMPORT && showNoValidAccountAlert && <AlertBox
              ...
              description={t('ui.Account.RestoreJson.allAccountsInFileExist')}
              title={t('ui.Account.RestoreJson.unableToImport')}
              type='warning'
            />
          }
            ...
            { stepState === StepState.UPLOAD_JSON_FILE &&
              ...
              >
                <Upload.SingleFileDragger
                  ...
                  hint={t('ui.Account.RestoreJson.dragAndDropPolkadotJsJson')}
                  onChange={onChangeFile}
                  statusHelp={fileValidateState.message}
                  title={t('ui.Account.RestoreJson.importByJsonFile')}
                />
              </Form.Item>
            }
              ...
              stepState === StepState.UPLOAD_JSON_FILE && requirePassword && (
                ...
                >
                  <div className='input-label'>
                    {t('ui.Account.RestoreJson.enterPolkadotJsPassword')}
                  </div>
                  <Input.Password
                    id={`${formName}_${passwordField}`}
                    onChange={onChangePassword}
                    placeholder={t('ui.Account.RestoreJson.password')}
