...
const Component: React.FC<ComponentProps> = ({ accountProxy, onBack, requestViewDerivedAccountDetails, requestViewDerivedAccounts }: ComponentProps) => {
  ...
  const filterTabItems = useMemo<FilterTabItemType[]>(() => {
    const result = [
      {
        label: t('ui.Account.Detail.accountAddress'),
    ...
    if (showDerivedAccounts) {
      result.push({
        label: t('ui.Account.Detail.derivedAccount'),
    ...
    if (showDerivationInfoTab) {
      result.push({
        label: t('ui.Account.Detail.derivationInfo'),
  ...
  const onDelete = useCallback(() => {
    alertModal.open({
      title: t('ui.Account.Detail.confirmation'),
      type: NotificationType.WARNING,
      content: t('ui.Account.Detail.removeAccountAccessWarning'),
      okButton: {
        text: t('ui.Account.Detail.remove'),
        onClick: () => {
          doDelete();
          alertModal.close();
        },
  ...
  const onCopyAddress = useCallback(() => {
    notify({
      message: t('ui.Account.Detail.copiedToClipboard')
    });
  }, [notify, t]);
  ...
  const accountNameValidator = useCallback(async (validate: RuleObject, value: string) => {
    ...
    if (value) {
      try {
        ...
        if (!isValid) {
          return Promise.reject(t('ui.Account.Detail.accountNameInUse'));
        }
      } catch (e) {
        return Promise.reject(t('ui.Account.Detail.accountNameInvalid'));
      }
    }
    ...
    if (![AccountProxyType.UNIFIED, AccountProxyType.SOLO].includes(accountProxy.accountType)) {
      return (
        <Button
          ...
          schema='error'
        >
          {t('ui.Account.Detail.deleteAccount')}
        </Button>
      );
    }
      ...
      <>
        ...
        >
          ...
          >
            <Input
              disabled={true}
              label={t('ui.Account.Detail.derivationPath')}
              placeholder={t('ui.Account.Detail.derivationPath')}
            />
          </Form.Item>
          ...
          >
            <Input
              disabled={true}
              label={t('ui.Account.Detail.parentAccount')}
              placeholder={t('ui.Account.Detail.parentAccount')}
            />
          </Form.Item>}
        </Form>
      </>
  ...
  return (
    <Layout.WithSubHeaderOnly
      ...
      title={t('ui.Account.Detail.accountDetails')}
    >
      <Form
      ...
      >
        <div className='account-field-wrapper'>
          ...
          <Form.Item
            ...
            rules={[
              {
                message: t('ui.Account.Detail.accountNameRequired'),
          ...
          >
            <Input
              ...
              disabled={false}
              label={t('ui.Account.Detail.accountName')}
              onBlur={form.submit}
              placeholder={t('ui.Account.Detail.accountName')}
              suffix={(
