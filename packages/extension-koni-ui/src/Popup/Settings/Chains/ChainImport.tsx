  ...
  const onSubmit = useCallback(() => {
    ...
    upsertChain(params)
      .then((result) => {
        ...
        if (result) {
          showNotification({
            message: t('ui.Settings.Chains.ChainImport.importedChainSuccessfully')
          });
          location?.useGoHome ? navigate(DEFAULT_ROUTER_PATH) : navigate(-1);
        } else {
          showNotification({
            message: t('ui.Settings.Chains.ChainImport.anErrorOccurredPleaseTryAgain')
          });
        }
      })
      .catch(() => {
  ...
  const blockExplorerValidator = useCallback((rule: RuleObject, value: string): Promise<void> => {
    return new Promise((resolve, reject) => {
      if (value.length === 0 || isUrl(value)) {
        ...
        reject(new Error(t('ui.Settings.Chains.ChainImport.blockExplorerMustBeValid')));
      }
    });
  }, [t]);
  ...
  const crowdloanUrlValidator = useCallback((rule: RuleObject, value: string): Promise<void> => {
    return new Promise((resolve, reject) => {
      if (value.length === 0 || isUrl(value)) {
        ...
        reject(new Error(t('ui.Settings.Chains.ChainImport.crowdloanUrlMustBeValid')));
      }
    });
  }, [t]);
  ...
  const handleErrorMessage = useCallback((errorCode: _CHAIN_VALIDATION_ERROR) => {
    switch (errorCode) {
      case _CHAIN_VALIDATION_ERROR.CONNECTION_FAILURE:
        return t('ui.Settings.Chains.ChainImport.cannotConnectToProvider');
      case _CHAIN_VALIDATION_ERROR.EXISTED_PROVIDER:
        return t('ui.Settings.Chains.ChainImport.providerAlreadyAdded');
      case _CHAIN_VALIDATION_ERROR.EXISTED_CHAIN:
        return t('ui.Settings.Chains.ChainImport.chainAlreadyAdded');
      default:
        return t('ui.Settings.Chains.ChainImport.errorValidatingProvider');
    }
  }, [t]);
    ...
    return new Promise((resolve, reject) => {
      if (isUrl(provider)) {
        ...
        validateCustomChain(parsedProvider)
          ...
          .catch(() => {
            setIsValidating(false);
            reject(new Error(t('ui.Settings.Chains.ChainImport.errorValidatingProvider')));
            setProviderValidation({ status: 'error', message: t('ui.Settings.Chains.ChainImport.errorValidatingProvider') });
          });
      } else {
        reject(new Error(t('ui.Settings.Chains.ChainImport.providerUrlNotValid')));
  ...
  return (
    <PageWrapper className={`chain_import ${className}`}>
      <Layout.WithSubHeaderOnly
        leftFooterButton={{
          onClick: onBack,
          children: t('ui.Settings.Chains.ChainImport.cancel')
        }}
        onBack={onBack}
        rightFooterButton={{
          block: true,
          ...
          loading: loading,
          onClick: onSubmit,
          children: t('ui.Settings.Chains.ChainImport.save')
        }}
        subHeaderIcons={[
          {
            icon: <InfoIcon />,
      ...
      >
        <div className={'chain_import__container'}>
          <div className={'chain_import__header_info'}>
            {t('ui.Settings.Chains.ChainImport.providerSupportInfo')}
          </div>
          <Form
            disabled={loading}
            form={form}
            ...
            <div className={'chain_import__attributes_container'}>
              ...
              >
                <Input
                  ...
                  placeholder={t('ui.Settings.Chains.ChainImport.providerUrl')}
                  prefix={(
                    <Icon
                      customSize={'24px'}
                      iconColor={token['gray-4']}
                  ...
                  tooltip={t('ui.Settings.Chains.ChainImport.providerUrl')}
              ...
              <Row gutter={token.paddingSM}>
                <Col span={16}>
                  <Form.Item name={'name'}>
                    <Input
                      disabled={true}
                      placeholder={t('ui.Settings.Chains.ChainImport.networkName')}
                      prefix={<Icon
                        customSize={'24px'}
                        iconColor={token['gray-4']}
                        phosphorIcon={Globe}
                        ...
                        weight={'bold'}
                      />}
                      tooltip={t('ui.Settings.Chains.ChainImport.networkName')}
                      tooltipPlacement={'topLeft'}
                    />
                  </Form.Item>
                </Col>
                <Col span={8}>
                  <Form.Item name={'symbol'}>
                    <Input
                      disabled={true}
                      placeholder={t('ui.Settings.Chains.ChainImport.symbol')}
                      tooltip={t('ui.Settings.Chains.ChainImport.symbol')}
                      tooltipPlacement={'topLeft'}
                    />
                  </Form.Item>
                </Col>
              ...
              <Row gutter={token.paddingSM}>
                <Col span={12}>
                  <Form.Item name={'priceId'}>
                    <Input
                      placeholder={t('ui.Settings.Chains.ChainImport.priceId')}
                      tooltip={t('ui.Settings.Chains.ChainImport.priceId')}
                      tooltipPlacement={'topLeft'}
                    />
                  </Form.Item>
                </Col>
                ...
                <Col span={12}>
                  <Form.Item name={'type'}>
                    <Input
                      disabled={true}
                      placeholder={t('ui.Settings.Chains.ChainImport.networkType')}
                      tooltip={t('ui.Settings.Chains.ChainImport.networkType')}
                      tooltipPlacement={'topLeft'}
                    />
                  </Form.Item>
                </Col>
              ...
              >
                <Input
                  placeholder={t('ui.Settings.Chains.ChainImport.blockExplorer')}
                  tooltip={t('ui.Settings.Chains.ChainImport.blockExplorer')}
                  tooltipPlacement={'topLeft'}
                />
              </Form.Item>
              ...
              >
                <Input
                  placeholder={t('ui.Settings.Chains.ChainImport.crowdloanUrl')}
                  tooltip={t('ui.Settings.Chains.ChainImport.crowdloanUrl')}
                  tooltipPlacement={'topLeft'}
                />
              </Form.Item>
            </div>
