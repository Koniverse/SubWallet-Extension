...
function Component ({ className }: Props): React.ReactElement<Props> {
  ...
  const footerBtn = useMemo((): ButtonProps => ({
    children: t('ui.NotFound.backToHome'),
    icon: (
      <Icon
        phosphorIcon={House}
        weight='fill'
  ...
  return (
    ...
    >
      ...
      <div className='sub-title h3-text'>{t('ui.NotFound.pageNotFound')}</div>
      <div className='h5-text description'>{t('ui.NotFound.invalidWebsiteUrl')}</div>
