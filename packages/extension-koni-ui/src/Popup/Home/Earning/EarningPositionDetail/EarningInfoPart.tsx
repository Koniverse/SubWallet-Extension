...
function Component ({ className, inputAsset, poolInfo }: Props) {
  ...
  return (
    <CollapsiblePanel
      ...
      title={t('ui.Earning.PositionDetail.EarningInfo.earningInfo')}
    >
      <MetaInfo
      ...
      >
        {!isSubnetStaking
          ? (
            <MetaInfo.Chain
              ...
              label={t('ui.Earning.PositionDetail.EarningInfo.network')}
            />
          )
          : (
            <MetaInfo.Default
              label={t('ui.Earning.PositionDetail.EarningInfo.subnet')}
            >
        ...
        {totalApy !== undefined && (
          <MetaInfo.Number
            label={t('ui.Earning.PositionDetail.EarningInfo.estimatedEarnings')}
            suffix={'% ' + t('ui.Earning.PositionDetail.EarningInfo.perYear')}
            value={totalApy}
        ...
        <MetaInfo.Number
          ...
          label={t('ui.Earning.PositionDetail.EarningInfo.minimumActiveStake')}
          suffix={inputAsset?.symbol}
          value={poolInfo.statistic?.earningThreshold.join || '0'}
          valueColorSchema='even-odd'
        />
        {unstakePeriod !== undefined && (
          <MetaInfo.Default label={t('ui.Earning.PositionDetail.EarningInfo.unstakingPeriod')}>
            {(poolInfo.type === YieldPoolType.LIQUID_STAKING || poolInfo.type === YieldPoolType.SUBNET_STAKING) && <span className={'__label'}>{t('ui.Earning.PositionDetail.EarningInfo.upTo')}</span>}
