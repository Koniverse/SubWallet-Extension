  ...
  return (
    <MetaInfo className={CN(className)}>
      <MetaInfo.DisplayType
        label={t('ui.History.Detail.Layout.transactionType')}
        typeName={t(TxTypeNameMap[data.type])}
      />
      <HistoryDetailHeader data={data} />
      <MetaInfo.Status
        label={t('ui.History.Detail.Layout.transactionStatus')}
      ...
      />
      <MetaInfo.Default label={t('ui.History.Detail.Layout.extrinsicHash')}>{extrinsicHash}</MetaInfo.Default>
      {!!data.time && (<MetaInfo.Default label={t('ui.History.Detail.Layout.submittedTime')}>{formatHistoryDate(data.time, language, 'detail')}</MetaInfo.Default>)}
      {!!data.blockTime && (<MetaInfo.Default label={t('ui.History.Detail.Layout.blockTime')}>{formatHistoryDate(data.blockTime, language, 'detail')}</MetaInfo.Default>)}
