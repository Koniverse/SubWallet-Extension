...
export const getEarningTimeText = (t: TFunction, hours?: number) => {
  if (hours !== undefined) {
    ...
    if (isDay) {
      ...
      unit = time > 1 ? t('ui.util.earning.days') : t('ui.util.earning.day');
    } else if (isHour) {
      time = hours;
      unit = time > 1 ? t('ui.util.earning.hours') : t('ui.util.earning.hour');
    } else {
      time = hours * 60;
      unit = time > 1 ? t('ui.util.earning.minutes') : t('ui.util.earning.minute');
    }

    return [time, unit].join(' ');
  } else {
    return t('ui.util.earning.unknownTime');
  }
};
