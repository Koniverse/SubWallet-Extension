...
const Component: React.FC<Props> = (props: Props) => {
  ...
  const balanceItems = useMemo<BalanceDisplayItem[]>(() => {
    if (isBitcoinChain && isBitcoinMetadata(metadata)) {
      return [
        { key: 'btc_transferable', label: t('ui.TokenItem.AccountBalance.btcTransferable'), value: free },
        { key: 'btc_rune', label: t('ui.TokenItem.AccountBalance.btcRuneLocked'), value: isBitcoinMetadata(metadata) ? String(metadata.runeBalance) : '0' },
        { key: 'btc_inscription', label: t('ui.TokenItem.AccountBalance.btcInscriptionLocked'), value: isBitcoinMetadata(metadata) ? String(metadata.inscriptionBalance) : '0' },
        { key: 'btc_total', label: t('ui.TokenItem.AccountBalance.total'), value: total }
      ];
    }

    return [
      { key: 'transferable', label: t('ui.TokenItem.AccountBalance.transferable'), value: free },
      { key: 'locked', label: t('ui.TokenItem.AccountBalance.locked'), value: locked }
    ];
    ...
    >
      ...
      {!!link && (
        <Button
          ...
          type={'ghost'}
        >
          {t('ui.TokenItem.AccountBalance.viewOnExplorer')}
        </Button>
      )}
    </MetaInfo>
  );
