...
import { BN_TEN, BN_ZERO, SWAP_FEES_MODAL } from '@subwallet/extension-koni-ui/constants';
...
const defaultTooltipMap: Record<SwapFeeType, (percentage?: number) => string> = {
  [SwapFeeType.PLATFORM_FEE]: () => detectTranslate('ui.Modal.Swap.Fees.thirdPartyFee'),
  [SwapFeeType.NETWORK_FEE]: () => detectTranslate('ui.Modal.Swap.Fees.transactionFee'),
  [SwapFeeType.WALLET_FEE]: (percentage?: number) => {
    if (!percentage) {
      return detectTranslate('ui.Modal.Swap.Fees.subWalletFee');
    } else {
      const message = detectTranslate('ui.Modal.Swap.Fees.percentageFee');
...
const Component: FC<Props> = (props: Props) => {
  ...
  const feeItems = useMemo(() => {
    ...
    const feeConfigs = [
      { type: SwapFeeType.NETWORK_FEE, label: t('ui.Modal.Swap.Fees.networkFee'), getTooltip: () => defaultTooltipMap[SwapFeeType.NETWORK_FEE]() },
      { type: SwapFeeType.PLATFORM_FEE, label: t('ui.Modal.Swap.Fees.providerFee'), getTooltip: () => defaultTooltipMap[SwapFeeType.PLATFORM_FEE]() },
      { type: SwapFeeType.WALLET_FEE, label: t('ui.Modal.Swap.Fees.subWalletFeeTitle'), getTooltip: (percentage?: number) => defaultTooltipMap[SwapFeeType.WALLET_FEE](percentage) }
    ];
    ...
    <SwModal
      ...
      footer={(
        <Button
          ...
          onClick={onCancel}
        >
          {t('ui.Modal.Swap.Fees.close')}
        </Button>
      )}
      id={modalId}
      onCancel={onCancel}
      title={t('ui.Modal.Swap.Fees.swapFees')}
    >
      <MetaInfo
      ...
      >
        <div className={CN('__list-container')}>
          ...
          <MetaInfo.Number
            ...
            decimals={0}
            label={t('ui.Modal.Swap.Fees.estimatedTotalFee')}
            prefix={(currencyData.isPrefix && currencyData.symbol) || ''}
            suffix={(!currencyData.isPrefix && currencyData.symbol) || ''}
            useNumberDisplay={true}
