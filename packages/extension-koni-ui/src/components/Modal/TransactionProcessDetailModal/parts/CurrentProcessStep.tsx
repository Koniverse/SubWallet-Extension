  ...
  const title = useMemo(() => {
    if (isStepCompleted(processData.status)) {
      if (processData.type === ProcessType.SWAP) {
        return t('ui.Modal.TxProcessDetail.CurrentStep.swapSuccess');
      } else if (processData.type === ProcessType.EARNING) {
        return t('ui.Modal.TxProcessDetail.CurrentStep.stakeSuccess');
      }

      return t('ui.Modal.TxProcessDetail.CurrentStep.transactionSuccess');
    }

    if (isStepFailed(processData.status)) {
      if (processData.type === ProcessType.SWAP) {
        return t('ui.Modal.TxProcessDetail.CurrentStep.swapFailed');
      } else if (processData.type === ProcessType.EARNING) {
        return t('ui.Modal.TxProcessDetail.CurrentStep.stakeFailed');
      }

      return t('ui.Modal.TxProcessDetail.CurrentStep.transactionFailed');
    }

    if (isStepTimeout(processData.status)) {
      return t('ui.Modal.TxProcessDetail.CurrentStep.transactionTimeout');
    }
    ...
    if (([
      ...
      return t('ui.Modal.TxProcessDetail.CurrentStep.transferTokenCrossChain');
    }

    if (currentStep.type === SwapStepType.SWAP) {
      return t('ui.Modal.TxProcessDetail.CurrentStep.swapToken');
    }
    ...
    if (([
      ...
      return t('ui.Modal.TxProcessDetail.CurrentStep.approveToken');
    }
    ...
    if (([
      ...
      return t('ui.Modal.TxProcessDetail.CurrentStep.stakeToken');
    }

    if (currentStep.type === SwapStepType.PERMIT) {
      return t('ui.Modal.TxProcessDetail.CurrentStep.signMessage');
    }
