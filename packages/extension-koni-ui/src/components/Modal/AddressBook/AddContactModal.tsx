...
const Component: React.FC<Props> = (props: Props) => {
  ...
  const addressValidator = useCallback((rule: RuleObject, address: string): Promise<void> => {
    if (!address) {
      return Promise.reject(new Error(t('ui.Modal.AddressBook.AddContact.contactAddressRequired')));
    }

    if (!isAddress(address)) {
      return Promise.reject(new Error(t('ui.Modal.AddressBook.AddContact.invalidContactAddress')));
    }
  ...
  const nameValidator = useCallback((rule: RuleObject, name: string): Promise<void> => {
    if (!name) {
      return Promise.reject(new Error(t('ui.Modal.AddressBook.AddContact.contactNameRequired')));
    }

    if (existNames.includes(name)) {
      return Promise.reject(new Error(t('ui.Modal.AddressBook.AddContact.contactNameMustBeUnique')));
    }
  ...
  return (
    <SwModal
      ...
      onCancel={onCancel}
      title={t('ui.Modal.AddressBook.AddContact.addContact')}
    >
      <Form
      ...
      >
        ...
        >
          <Input
            label={t('ui.Modal.AddressBook.AddContact.contactName')}
            prefix={(
              <AccountProxyAvatar
        ...
        >
          <Input
            ...
            label={t('ui.Modal.AddressBook.AddContact.contactAddress')}
            prefix={
        ...
        >
          <Button
            ...
            loading={loading}
          >
            {t('ui.Modal.AddressBook.AddContact.addContact')}
          </Button>
        </Form.Item>
      </Form>
    </SwModal>
