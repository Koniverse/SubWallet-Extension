...
const Component: React.FC<Props> = (props: Props) => {
  ...
  const modalProps: SwModalFuncProps = useMemo(() => {
    return {
      closable: true,
      content: t('ui.Modal.AddressBook.EditContact.deleteContactWarning'),
      id: DELETE_ADDRESS_BOOK_MODAL,
      okText: t('ui.Modal.AddressBook.EditContact.remove'),
      subTitle: t('ui.Modal.AddressBook.EditContact.deleteThisContact'),
      title: t('ui.Modal.AddressBook.EditContact.confirmation'),
      type: 'error',
      maskClosable: true,
      zIndex: 1005
    };
  ...
  const nameValidator = useCallback((rule: RuleObject, name: string): Promise<void> => {
    if (!name) {
      return Promise.reject(new Error(t('ui.Modal.AddressBook.EditContact.contactNameRequired')));
    }

    if (existNames.includes(name)) {
      return Promise.reject(new Error(t('ui.Modal.AddressBook.EditContact.contactNameMustBeUnique')));
    }
  ...
  return (
    <SwModal
      ...
      title={t('ui.Modal.AddressBook.EditContact.editContact')}
    >
      <Form
      ...
      >
        ...
        >
          <Input
            label={t('ui.Modal.AddressBook.EditContact.contactName')}
            prefix={(
              <AccountProxyAvatar
        ...
        <Form.Item>
          <Field
            ...
            label={t('ui.Modal.AddressBook.EditContact.contactAddress')}
            suffix={(
              <Button
        ...
        >
          ...
          <Button
            ...
            schema='secondary'
          >
            {t('ui.Modal.AddressBook.EditContact.cancel')}
          </Button>
          <Button
            block={true}
            ...
            loading={loading}
          >
            {t('ui.Modal.AddressBook.EditContact.save')}
          </Button>
        </Form.Item>
      </Form>
    </SwModal>
