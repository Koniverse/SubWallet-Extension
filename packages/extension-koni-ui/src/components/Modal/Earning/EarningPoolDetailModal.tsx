...
function Component ({ chain, className, detailItem, maxPoolMembersValue, onCancel }: Props): React.ReactElement<Props> {
  ...
  return (
    <SwModal
      ...
      onCancel={onCancel}
      title={t('ui.Modal.Earning.PoolDetail.poolDetails')}
    >
      <MetaInfo
      ...
      >
        <MetaInfo.Account
          address={address}
          label={t('ui.Modal.Earning.PoolDetail.pool')}
          name={name}
...

        {
          state && (
            <MetaInfo.Status
              label={t('ui.Modal.Earning.PoolDetail.state')}
        ...
        <MetaInfo.Status
          label={t('ui.Modal.Earning.PoolDetail.earningStatus')}
        ...
        <MetaInfo.Number
          decimals={decimals}
          label={t('ui.Modal.Earning.PoolDetail.totalStaked')}
          suffix={symbol}
        ...
        {!maxPoolMembersValue &&
            <MetaInfo.Number
              label={t('ui.Modal.Earning.PoolDetail.member')}
              value={memberCounter}
              valueColorSchema={'even-odd'}
            />}
