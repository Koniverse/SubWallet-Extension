...
function Component ({ address, className = '', items, onSelectItem }: Props): React.ReactElement<Props> {
  ...
  const confirmModalProps = useMemo((): SwModalFuncProps => ({
    ...
    title: t<string>('ui.Modal.Receive.TokensSelector.unsupportedNetwork'),
    maskClosable: true,
    closable: true,
    subTitle: t<string>('ui.Modal.Receive.TokensSelector.confirmGetAddress'),
    okText: t<string>('ui.Modal.Receive.TokensSelector.getAddress'),
    okCancel: true,
    type: 'warn',
    cancelButtonProps: {
      children: t<string>('ui.Modal.Receive.TokensSelector.cancel'),
      schema: 'secondary'
    },
    ...
    return () => {
      if (ledgerCheck !== 'unnecessary' && !ledgerGenericAllowNetworks.includes(item.originChain)) {
        handleSimpleConfirmModal({
          content: t('ui.Modal.Receive.TokensSelector.ledgerIncompatibleNetworkWarning', {
              replace: {
  ...
  return (
    ...
    >
      <SwList.Section
        ...
        searchPlaceholder={t<string>('ui.Modal.Receive.TokensSelector.searchToken')}
      />
    </SwModal>
  );
}
