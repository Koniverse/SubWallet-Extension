...
const Component: React.FC<Props> = ({ address, className, selectedNetwork }: Props) => {
  ...
  const onClickCopyBtn = useCallback(() => notify({ message: t('ui.Modal.Receive.Qr.copiedToClipboard') }), [notify, t]);
  ...
  return (
    <SwModal
      ...
      title={t<string>('ui.Modal.Receive.Qr.yourAddress')}
    >
      <>
            ...
            rightItem={
              <CopyToClipboard text={formattedAddress}>
                <Button
                  ...
                  size='xs'
                  tooltip={t('ui.Modal.Receive.Qr.copyAddress')}
                  type='ghost'
                />
              </CopyToClipboard>
            }
        ...
        >{t('ui.Modal.Receive.Qr.viewOnExplorer')}</Button>
      </>
    </SwModal>
  );
};
