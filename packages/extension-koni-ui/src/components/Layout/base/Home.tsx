...
const Component = ({ children, className, isDisableHeader, onClickSearchToken, showFaderIcon, showNotificationIcon, showSearchToken, showSidebarIcon, showTabBar }: Props) => {
  ...
  const faderMenu = useMemo(() => {
    return [
      {
        key: '1',
        label: t('ui.Layout.Home.networkDisplay'),
        icon: (
      ...
      {
        key: '2',
        label: t('ui.Layout.Home.searchToken'),
        icon: (
  ...
  const sidebarMenu = useMemo(() => {
    const expandViewItem = {
      key: '1',
      label: t('ui.Layout.Home.expandView'),
      icon: (
    ...
    const openInSidebarItem = {
      key: '2',
      label: t('ui.Layout.Home.openInSidebar'),
      icon: (
    ...
    const openInPopupItem = {
      key: '3',
      label: t('ui.Layout.Home.openInPopup'),
      icon: (
        <Icon
  ...
  const headerIcons = useMemo<ButtonProps[]>(() => {
    ...
    if (showNotificationIcon) {
      icons.push({
        ...
        tooltip: t('ui.Layout.Home.notifications'),
    ...
    if (showFaderIcon) {
      if (showSearchToken) {
        ...
        icons.push({
          ...
          onClick: onOpenCustomizeModal,
          tooltip: t('ui.Layout.Home.customizeAssetDisplay'),
          tooltipPlacement: 'bottomRight'
        });
      }
    }
    ...
    if (showSidebarIcon) {
      if (isExpanseMode) {
        icons.push({
          ...
          tooltip: t('ui.Layout.Home.openInSidebar'),
          tooltipPlacement: 'bottomRight'
        });
      } else {
        icons.push({
