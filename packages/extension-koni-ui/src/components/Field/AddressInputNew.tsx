...
function Component (props: Props, ref: ForwardedRef<AddressInputRef>): React.ReactElement<Props> {
  ...
  const autoCompleteOptions = useMemo<AutoCompleteGroupItem[]>(() => {
    ...
    if (walletItems.length) {
      ...
      result.push(genAutoCompleteGroupItem(t('ui.Field.AddressInputNew.myWallet'), walletItems.map((i) => genAutoCompleteItem(i))));
    }

    if (contactItems.length) {
      contactItems.sort(sortFuncAnalyzeAddress);
      result.push(genAutoCompleteGroupItem(t('ui.Field.AddressInputNew.myContact'), contactItems.map((i) => genAutoCompleteItem(i))));
    }

    if (domainItems.length) {
      domainItems.sort(sortFuncAnalyzeAddress);
      result.push(genAutoCompleteGroupItem(t('ui.Field.AddressInputNew.domainName'), domainItems.map((i) => genAutoCompleteItem(i))));
    }

    if (recentItems.length) {
      recentItems.sort(sortFuncAnalyzeAddress);
      result.push(genAutoCompleteGroupItem(t('ui.Field.AddressInputNew.recent'), recentItems.map((i) => genAutoCompleteItem(i))));
    }
  ...
  const onSwitchAdvancedAddressDetection = useCallback((checked: boolean) => {
    if (checked) {
      alertModal.open({
        closable: false,
        title: t('ui.Field.AddressInputNew.advancedAddressConversion'),
        type: NotificationType.WARNING,
        content: t('ui.Field.AddressInputNew.advancedAddressConversionInfo'),
        cancelButton: {
          text: t('ui.Field.AddressInputNew.cancel'),
          icon: XCircle,
          iconWeight: 'fill',
          onClick: () => {
            // setOpenDropdownManually(undefined);
        ...
        okButton: {
          text: t('ui.Field.AddressInputNew.enable'),
          icon: CheckCircle,
          iconWeight: 'fill',
          onClick: () => {
            // setOpenDropdownManually(undefined);
          ...
          isShowAdvancedAddressDetection && (
            <div className={'__advanced-address-detection'}>
              ...
              <div className={'__advanced-address-detection-label'}>
                {t('ui.Field.AddressInputNew.advancedAddressDetection')}
              </div>
      ...
      <div className={CN(className, '-input-container')}>
        ...
        >
          <Input
            ...
            disabled={disabled}
            id={id}
            label={label || t('ui.Field.AddressInputNew.accountAddress')}
            placeholder={placeholder || t('ui.Field.AddressInputNew.typeOrPasteAddress')}
            prefix={
              <>
                {
                  selectedOption && (
