...
const Component = (props: Props, ref: ForwardedRef<InputRef>): React.ReactElement<Props> => {
  ...
  const footerButton = useCallback(() => {
    return (
      ...
      >
        {t('ui.Field.ProviderSelector.addNewProvider')}
      </Button>
    );
  ...
  return (
    <SelectModal
      ...
      label={label}
      onSelect={onSelect}
      placeholder={placeholder || t('ui.Field.ProviderSelector.selectProvider')}
      prefix={(
        <Icon
          customSize={'24px'}
      ...
      title={label || placeholder || t('ui.Field.ProviderSelector.selectProvider')}
    />
  );
};
