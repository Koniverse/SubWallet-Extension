...
const Component = (props: Props, ref: ForwardedRef<InputRef>) => {
  ...
  const sortingOptions: SortOption[] = useMemo(() => {
    const result: SortOption[] = [
      {
        desc: false,
        label: t('ui.Field.Earning.ValidatorSelector.lowestCommission'),
    ...
    if (hasReturn) {
      result.push({
        desc: true,
        label: t('ui.Field.Earning.ValidatorSelector.highestAnnualReturn'),
    ...
    if (nominations && nominations.length > 0) {
      result.push({
        desc: true,
        label: t('ui.Field.Earning.ValidatorSelector.nomination'),
    ...
    result.push({
      desc: false,
      label: t('ui.Field.Earning.ValidatorSelector.lowestMinActiveStake'),
  ...
  const applyLabel = useMemo(() => {
    ...
    if (!fewValidators) {
      switch (label) {
        case 'dApp':
          return detectTranslate('ui.Field.Earning.ValidatorSelector.applyOneDapp');
        case 'Collator':
          return detectTranslate('ui.Field.Earning.ValidatorSelector.applyOneCollator');
        case 'Validator':
          return detectTranslate('ui.Field.Earning.ValidatorSelector.applyOneValidator');
      }
    } else {
      switch (label) {
        case 'dApp':
          return detectTranslate('ui.Field.Earning.ValidatorSelector.applyNumberDapps');
        case 'Collator':
          return detectTranslate('ui.Field.Earning.ValidatorSelector.applyNumberCollators');
        case 'Validator':
          return detectTranslate('ui.Field.Earning.ValidatorSelector.applyNumberValidators');
      }
    }
  }, [chain, fewValidators]);
  ...
  return (
    <>
      <SelectValidatorInput
        ...
        label={t('ui.Field.Earning.ValidatorSelector.select') + ' ' + t(handleValidatorLabel)}
        loading={loading}
      ...
      <SwModal
        ...
        title={t('ui.Field.Earning.ValidatorSelector.select') + ' ' + t(handleValidatorLabel)}
      >
        <SwList.Section
