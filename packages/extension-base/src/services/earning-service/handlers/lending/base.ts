...
import { BasicTxErrorType, LendingYieldPoolInfo, StakingTxErrorType, YieldPoolType } from '@subwallet/extension-base/types';
...
export default abstract class BaseLendingPoolHandler extends BaseSpecialStakingPoolHandler {
  ...
  async validateYieldLeave (amount: string, address: string, fastLeave: boolean, selectedTarget?: string): Promise<TransactionError[]> {
    ...
    if (bnAmount.lt(minUnstake)) {
      ...
      errors.push(new TransactionError(StakingTxErrorType.NOT_ENOUGH_MIN_UNSTAKE, t('bg.EARNING.services.service.earning.lending.unstakeMinimumAmount', { replace: { amount: minUnstakeStr, token: derivativeTokenInfo.symbol } })));
    }
