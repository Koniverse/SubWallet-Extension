...
export default class TaoNativeStakingPoolHandler extends BaseParaStakingPoolHandler {
    ...
    if (new BigN(amount).lt(minUnstake)) {
      return [new TransactionError(BasicTxErrorType.INVALID_PARAMS, t('bg.EARNING.services.service.earning.nativeStaking.tao.unstakeAmountTooLow', {
        replace: {
          ...
          symbol: poolInfo.metadata.subnetData?.subnetSymbol || _getAssetSymbol(this.nativeToken)
        }
      }))];
    }
  ...
  override async handleChangeEarningValidator (data: SubmitBittensorChangeValidatorStaking): Promise<TransactionData> {
    ...
    if (new BigN(amount).lte(0)) {
      return Promise.reject(new TransactionError(BasicTxErrorType.INVALID_PARAMS, t('bg.SWAP.background.error.Swap.amountMustBeGreaterThanZero')));
    }
    ...
    if (!(maxAmount === amount) && new BigN(maxAmount).minus(new BigN(amount)).lt(bnMinMoveStake)) {
      return Promise.reject(new TransactionError(StakingTxErrorType.REMAINING_AMOUNT_TOO_LOW,
        t('bg.EARNING.services.service.earning.nativeStaking.tao.remainingStakeBelowMinimumWarning', {
          replace: {
            maxAmount: formatNumber(maxAmount, _getAssetDecimals(this.nativeToken)),
            subnetSymbol: metadata?.subnetSymbol || _getAssetSymbol(this.nativeToken)
          }
        })
      ));
    }
