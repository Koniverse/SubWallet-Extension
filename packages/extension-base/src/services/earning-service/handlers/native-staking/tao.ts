...
export default class TaoNativeStakingPoolHandler extends BaseParaStakingPoolHandler {
  ...
  public override async validateYieldJoin (data: SubmitJoinNativeStaking, path: OptimalYieldPath): Promise<TransactionError[]> {
    ...
    if (new BigN(amount).lt(new BigN(DEFAULT_DTAO_MINBOND))) {
      return [new TransactionError(BasicTxErrorType.INVALID_PARAMS, t('Insufficient stake. You need to stake at least {{amount}} {{symbol}} to earn rewards', { replace: { amount: formatNumber(DEFAULT_DTAO_MINBOND, _getAssetDecimals(this.nativeToken)), symbol: _getAssetSymbol(this.nativeToken) } }))];
    }
  ...
  public override async validateYieldLeave (amount: string, address: string, fastLeave: boolean, selectedTarget?: string, slug?: string, poolInfo?: YieldPoolInfo): Promise<TransactionError[]> {
    ...
    if (new BigN(amount).lt(bnMinUnstake)) {
      return [new TransactionError(BasicTxErrorType.INVALID_PARAMS, t('Amount too low. You need to unstake at least {{amount}} {{symbol}}', { replace: { amount: formatNumber(bnMinUnstake, _getAssetDecimals(this.nativeToken)), symbol: _getAssetSymbol(this.nativeToken) } }))];
    }
