...
export default class EvmRequestHandler {
  ...
  public async addConfirmation<CT extends ConfirmationType> (
    ...
    if (duplicated) {
      throw new EvmProviderError(EvmProviderErrorType.INVALID_PARAMS, t('bg.service.request.EvmHandler.duplicateRequest'));
    }
  ...
  public updateConfirmation<CT extends ConfirmationType> (
    ...
    if (!exists) {
      throw new EvmProviderError(EvmProviderErrorType.INVALID_PARAMS, t('bg.service.request.EvmHandler.requestDoesNotExist'));
    }
  ...
  private async signMessage (confirmation: ConfirmationDefinitions['evmSignatureRequest'][0]): Promise<string> {
    ...
    switch (type) {
      ...
      default:
        throw new EvmProviderError(EvmProviderErrorType.INVALID_PARAMS, t('bg.service.request.EvmHandler.unsupportedAction'));
    }
  }
  ...
  public async completeConfirmation (request: RequestConfirmationComplete): Promise<boolean> {
    ...
    for (const ct in request) {
      ...
      if (!resolver || !confirmation) {
        this.#logger.error(t('bg.service.request.EvmHandler.unableToProceed'), type, id);
        throw new Error(t('bg.service.request.EvmHandler.unableToProceed'));
      }
