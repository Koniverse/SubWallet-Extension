...
export function validateNativeLocalTransferMetadata (chainAsset: _ChainAsset) {
  ...
  const onChainInfoLocalGroup = [_TRANSFER_CHAIN_GROUP.centrifuge, ..._TRANSFER_CHAIN_GROUP.acala, ..._TRANSFER_CHAIN_GROUP.kintsugi, 'pendulum', 'amplitude', 'bifrost', 'bifrost_dot'];
  const onChainInfoNativeGroup = [_TRANSFER_CHAIN_GROUP.centrifuge, ..._TRANSFER_CHAIN_GROUP.acala, ..._TRANSFER_CHAIN_GROUP.kintsugi];
  const assetIdLocalGroup = [..._TRANSFER_CHAIN_GROUP.statemine, ...moonbeamGroup, 'hydradx'];
