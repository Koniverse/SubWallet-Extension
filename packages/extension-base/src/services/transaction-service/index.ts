...
import { _getAssetDecimals, _getAssetSymbol, _getChainNativeTokenBasicInfo, _getChainNativeTokenSlug, _getEvmChainId, _isChainEvmCompatible, _isNativeTokenBySlug } from '@subwallet/extension-base/services/chain-service/utils';
...
export default class TransactionService {
  ...
  public async validateTransaction (transactionInput: SWTransactionInput): Promise<SWTransactionResponse> {
    ...
    if (!chainInfo) {
      validationResponse.errors.push(new TransactionError(BasicTxErrorType.INTERNAL_ERROR, t('bg.koni.handler.Extension.cannotFindNetwork')));
    }
      ...
      this.state.requestService.addConfirmation(id, url || EXTENSION_REQUEST_URL, 'evmSendTransactionRequest', payload, { isPassConfirmation })
        .then(async ({ isApproved, payload }) => {
          if (isApproved) {
            ...
            if (!payload) {
              throw new EvmProviderError(EvmProviderErrorType.UNAUTHORIZED, t('bg.DAPP.background.error.EvmProvider.failedToSign'));
            }
    ...
    this.state.requestService.addConfirmationCardano(id, url || EXTENSION_REQUEST_URL, 'cardanoSendTransactionRequest', transactionConfig, {})
      .then(({ isApproved, payload }) => {
        if (!isApproved) {
          ...
          if (!payload) {
            throw new Error('bg.DAPP.background.error.EvmProvider.failedToSign');
          }
  ...
  private handleTransactionTimeout (emitter: EventEmitter<TransactionEventMap>, eventData: TransactionEventResponse): void {
    const timeout = setTimeout(() => {
      ...
      if (transaction.status !== ExtrinsicStatus.SUCCESS && transaction.status !== ExtrinsicStatus.FAIL) {
        eventData.errors.push(new TransactionError(BasicTxErrorType.TIMEOUT, t('bg.TRANSACTION.background.error.Transaction.transactionTimeout')));
