...
import { externalMessage, sendTonTransaction } from '@subwallet/extension-base/services/balance-service/helpers/subscribe/ton/utils';
...
import { AccountJson, LeavePoolAdditionalData, RequestStakePoolingBonding, RequestYieldStepSubmit, SpecialYieldPoolInfo, YieldPoolType } from '@subwallet/extension-base/types';
...
export default class TransactionService {
  ...
  private async signAndSendTonTransaction ({ address, chain, id, transaction, url }: SWTransaction): Promise<TransactionEmitter> {
    ...
    walletContract.createTransfer({
      ...
      sendTonTransaction(boc).then((externalMsgHash) => {
        if (!externalMsgHash) {
          return;
        }

        console.log('externalMsgHash', externalMsgHash);
