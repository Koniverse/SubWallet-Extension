...
export default class TransactionService {
  ...
  public async validateTransaction (transactionInput: SWTransactionInput): Promise<SWTransactionResponse> {
    ...
    if (!chainInfo) {
      validationResponse.errors.push(new TransactionError(BasicTxErrorType.INTERNAL_ERROR, t('bg.service.transaction.cannotFindNetwork')));
    }
      ...
      this.state.requestService.addConfirmation(id, url || EXTENSION_REQUEST_URL, 'evmSendTransactionRequest', payload, { isPassConfirmation })
        .then(async ({ isApproved, payload }) => {
          if (isApproved) {
            ...
            if (!payload) {
              throw new EvmProviderError(EvmProviderErrorType.UNAUTHORIZED, t('bg.service.transaction.failedToSign'));
            }
            ...
            if (!isExternal) {
              ...
              if (recover.toLowerCase() !== account.address.toLowerCase()) {
                throw new EvmProviderError(EvmProviderErrorType.UNAUTHORIZED, t('bg.service.transaction.wrongSignatureSignWithDappAccount'));
              }
  ...
  private handleTransactionTimeout (emitter: EventEmitter<TransactionEventMap>, eventData: TransactionEventResponse): void {
    const timeout = setTimeout(() => {
      ...
      if (transaction.status !== ExtrinsicStatus.SUCCESS && transaction.status !== ExtrinsicStatus.FAIL) {
        eventData.errors.push(new TransactionError(BasicTxErrorType.TIMEOUT, t('bg.service.transaction.transactionTimeout')));
