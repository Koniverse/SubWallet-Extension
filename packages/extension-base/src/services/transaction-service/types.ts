...
export interface SWTransactionBase extends ValidateTransactionResponse, Partial<Pick<BaseRequestSign, 'ignoreWarnings' | 'substrateProxyAddress'>>, TransactionFee, SWTransactionEmitter {
  id: string;
  url?: string;
  isInternal: boolean,
  chain: string;
...
export interface SWTransactionInput extends SwInputBase, Partial<Pick<SWTransactionBase, 'estimateFee' | 'signAfterCreate' | 'isPassConfirmation' | 'step' | 'errorOnTimeOut' | 'xcmFeeDryRun' | 'substrateProxyAddress'>>, TransactionFee {
  id?: string;
