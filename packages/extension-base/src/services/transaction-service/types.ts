...
export interface SWTransactionBase extends ValidateTransactionResponse, BaseRequestSign, TransactionFee, SWTransactionEmitter {
  id: string;
  url?: string;
  isInternal: boolean,
  chain: string;
...
export interface SWTransactionInput extends SwInputBase, Partial<Pick<SWTransactionBase, 'estimateFee' | 'signAfterCreate' | 'isPassConfirmation' | 'step' | 'errorOnTimeOut' | 'xcmFeeDryRun' | 'signerSubstrateMultisigAddress' | 'multisigAccountInfo'>>, TransactionFee {
  id?: string;
