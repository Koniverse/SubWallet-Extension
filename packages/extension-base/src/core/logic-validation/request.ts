    ...
    try {
      ...
      if (!payload.pair) {
        ...
        if (payload.pair.meta.noPublicKey) {
          return handleAuthError(payload, t('bg.validation.request.accountNotSupportedForAction'), 'dApp', errors);
        }
...
export async function validationBitcoinSignMessageMiddleware (koni: KoniState, url: string, payload_: PayloadValidated): Promise<PayloadValidated> {
  ...
  if (address === '' || !message) {
    handleError(t('bg.validation.request.notFoundAddressOrPayload'));
  }

  if (!isBitcoinAddress(address)) {
    handleError(t('bg.validation.request.invalidBitcoinAddress'));
  }

  const pair = pair_ || keyring.getPair(address);

  if (!pair) {
    handleError(t('bg.validation.request.unableToFindAccount'));
  }
...
export async function validationBitcoinSignPsbtMiddleware (koni: KoniState, url: string, payload_: PayloadValidated): Promise<PayloadValidated> {
  ...
  if (!(psbtParams.network === 'mainnet' || psbtParams.network === 'testnet')) {
    handleError(t('bg.validation.request.networkMustBeMainnetOrTestnet'));
  }

  if (!networkKey) {
    handleError(t('bg.validation.request.networkUnavailableSwitchOrAdd'));
  }

  if (!psbt || !address) {
    handleError(t('bg.validation.request.notFoundPayloadToSign'));
  }

  if (!isHex(`0x${psbt}`)) {
    handleError(t('bg.validation.request.psbtMustBeHexEncoded'));
  }

  if (!isBitcoinAddress(address)) {
    handleError(t('bg.validation.request.notFoundAddress'));
  }

  const addressInfo = getBitcoinAddressInfo(address);

  if (psbtParams.network !== addressInfo.network) {
    handleError(t('bg.validation.request.accountOrNetworkNotMatched'));
  }
...
export async function validationBitcoinSendTransactionMiddleware (koni: KoniState, url: string, payload_: PayloadValidated): Promise<PayloadValidated> {
  ...
  if (!transactionParams.recipients?.length) {
    handleError(t('bg.validation.request.provideRecipientAndAmount'));
  }

  if (transactionParams.recipients?.length > 1) {
    handleError(t('bg.validation.request.multipleRecipientsNotSupported'));
  }
  ...
  if (recipientAccountInfo.network !== transactionParams.network) {
    handleError(t('bg.validation.request.invalidRecipientAddress'));
  }

  if (transactionParams.recipients.length !== 1) {
    handleError(t('bg.validation.request.receivingAddressMustBeSingleAccount'));
  }

  if (address === transactionParams.recipients[0].address) {
    handleError(t('bg.validation.request.mustBeDifferentFromSendingAddress'));
  }

  const pair = pair_ || keyring.getPair(address);

  if (!pair) {
    handleError(t('bg.validation.request.unableToFindAccountError'));
  }
  ...
  try {
    ...
    if (message.toLowerCase().includes(t('bg.validation.request.pleaseEnableNetwork'))) {
  ...
  if (new BigN(freeBalance.value).lte(totalValue)) {
    handleError(t('bg.validation.request.insufficientBalanceError'));
  }
...
export function convertErrorMessage (message_: string, name?: string): string[] {
  ...
  if (
    ...
    return [t('bg.validation.request.reEnableOrChangeRpc'), t('bg.validation.request.unstableNetworkConnection')];
  }

  if (message.includes('network is currently not supported')) {
    return [t('bg.validation.request.networkNotSupportedImport'), t('bg.validation.request.networkNotSupported')];
  }
  ...
  if (message.includes('recipient address not found')) {
    return [t('bg.validation.request.enterRecipientAddress'), t('bg.validation.request.recipientAddressNotFound')];
  }

  if (message.includes('is not a number') || message.includes('invalid number value') || message.includes('invalid bignumberish')) {
    return [t('bg.validation.request.amountMustBeInteger'), t('bg.validation.request.invalidAmount')];
  }

  if (message.includes('calculate estimate gas fee') || message.includes('invalidcode')) {
    return [t('bg.validation.request.gasCalculationError'), t('bg.validation.request.gasCalculationErrorTitle')];
  }

  if (message.includes('invalid recipient address')) {
    return [t('bg.validation.request.recipientAddressTypeMismatch'), t('bg.validation.request.invalidRecipientAddressError')];
  }

  if (message.includes('must be different from sending address')) {
    return [t('bg.validation.request.recipientMustBeDifferentFromSender'), t('bg.validation.request.invalidRecipientAddressError')];
  }

  if (message.includes('the sender address must be the ethereum address type')) {
    return [t('bg.validation.request.senderMustBeEthereumAddress'), t('bg.validation.request.invalidAddressType')];
  }

  if (message.includes('the sender address must be the ethereum address type')) {
    return [t('bg.validation.request.senderMustBeBitcoinAddress'), t('bg.validation.request.invalidAddressType')];
  }

  if (message.includes('account or the network is not matched')) {
    return [t('bg.validation.request.accountDoesNotMatchNetwork'), t('bg.validation.request.invalidAddressType')];
  }

  if (message.includes('receiving address must be a single account')) {
    return [t('bg.validation.request.receivingAddressMustBeSingle'), t('bg.validation.request.invalidRecipientAddressError')];
  }

  if (message.includes('insufficient balance') || message.includes('insufficient funds')) {
    return [t('bg.validation.request.insufficientSenderBalance'), t('bg.validation.request.unableToSignTransaction')];
  }

  // Sign Message
  if (message.includes('not found address or payload to sign')) {
    return [t('bg.validation.request.errorSigningRequest'), t('bg.validation.request.unableToSign')];
  }

  if (message.includes('unsupported method') || message.includes('unsupported action')) {
    return [t('bg.validation.request.signMethodNotSupported'), t('bg.validation.request.methodNotSupported')];
  }

  if (message.includes('eip712 typed data') || message.includes('invalid message')) {
    return [t('bg.validation.request.errorAttemptingToSign'), t('bg.validation.request.unableToSign')];
  }
