...
const defaultErrorMap: Record<BalanceErrorType, { message: string, code?: number }> = {
  [BalanceErrorType.NETWORK_ERROR]: {
    message: detectTranslate('bg.BALANCE.background.error.Balance.networkInactiveEnable'),
    code: undefined
  },
  [BalanceErrorType.TOKEN_ERROR]: {
    message: detectTranslate('bg.BALANCE.background.error.Balance.tokenNotSupported'),
    code: undefined
  },
  [BalanceErrorType.TIMEOUT]: {
    message: detectTranslate('bg.BALANCE.background.error.Balance.unableToGetBalanceReEnableNetwork'),
    code: undefined
  },
