...
export class KoniCron {
  ...
  start = async () => {
    ...
    this.eventHandler = (events, eventTypes) => {
      ...
      if (this.checkNetworkAvailable(serviceInfo)) { // only add cron jobs if there's at least 1 active network
        ...
        (commonReload || needUpdateNft) && this.addCron('detectNft', this.detectEvmCollectionNft(address), CRON_REFRESH_NFT_DETECT_INTERVAL);
    ...
    if (Object.keys(this.state.getSubstrateApiMap()).length !== 0 || Object.keys(this.state.getEvmApiMap()).length !== 0) {
      ...
      this.addCron('detectNft', this.detectEvmCollectionNft(currentAccountInfo.proxyId), CRON_REFRESH_NFT_DETECT_INTERVAL);
  ...
  public async reloadNft () {
    ...
    this.addCron('detectNft', this.detectEvmCollectionNft(address), CRON_REFRESH_NFT_DETECT_INTERVAL);
