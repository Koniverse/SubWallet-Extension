...
import { encrypt as simpleEncrypt, decrypt as simpleDecrypt } from '@subwallet/extension-base/utils/simpleCrypto';
...
export default class KoniState {
  ...
  public async saveSubscanApiKey (apiKey: string): Promise<boolean> {
    ...
    // Get cipher password used for encryption
      ...
      const encryptedData = simpleEncrypt(cipherPassword, { apiKey });
  ...
  public async getSubscanApiKey (): Promise<string | null> {
    ...
    // Read encrypted API key from storage
      ...
      const decryptedData = simpleDecrypt(cipherPassword, JSON.parse(encryptedData)) as { apiKey?: string };
