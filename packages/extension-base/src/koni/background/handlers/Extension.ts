...
import { dynamicTaoSlug, isDynamicStaking } from '@subwallet/extension-base/services/earning-service/utils';
...
export default class KoniExtension {
  ...
  private async makeCrossChainTransfer (inputData: RequestCrossChainTransfer): Promise<SWTransactionResponse> {
    const { destinationNetworkKey, feeCustom, feeOption, from, isPassConfirmation, nonNativeTokenPayFeeSlug, originNetworkKey, to, tokenSlug, transferAll, transferBounceable, value } = inputData;
  ...
  private async getYieldPoolTargets (request: RequestGetYieldPoolTargets): Promise<ResponseGetYieldPoolTargets> {
    ...
    if (isDynamicStaking(slug)) {
      targets = await this.#koniState.earningService.getPoolTargets(dynamicTaoSlug);
    } else {
      targets = await this.#koniState.earningService.getPoolTargets(slug);
    }
