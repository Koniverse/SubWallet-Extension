...
export default class KoniExtension {
  ...
  private async handleYieldChangeValidator (params: SubmitYieldJoinData) {
    const { address, slug } = params;
  ...
  public async handle<TMessageType extends MessageTypes> (id: string, type: TMessageType, request: RequestTypes[TMessageType], port: chrome.runtime.Port): Promise<ResponseType<TMessageType>> {
    ...
    switch (type) {
      ...
      case 'pri(yield.changeValidator.submit)':
        return await this.handleYieldChangeValidator(request as SubmitYieldJoinData);
        /* Others */
