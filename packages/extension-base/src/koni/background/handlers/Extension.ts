...
export default class KoniExtension {
  ...
  private metadataApprove ({ id }: RequestMetadataApprove): boolean {
    ...
    assert(queued, t('bg.handler.Extension.unableToProceed'));
  ...
  private saveRecentAccount ({ accountId, chain }: RequestSaveRecentAccount): KeyringAddress {
    if (isAddress((accountId))) {
      ...
      throw Error(t('bg.handler.Extension.thisIsNotAnAddress'));
    }
  }
  ...
  private async makeTransfer (inputData: RequestSubmitTransfer): Promise<SWTransactionResponse> {
    ...
    try {
      ...
      if (error.message.includes('transfer amount exceeds balance')) {
        error.message = t('bg.handler.Extension.insufficientBalance');
      }
  ...
  private async makePsbtTransferAfterConfirmation (inputData_: RequestSubmitSignPsbtTransfer): Promise<SWTransactionResponse> {
    ...
    if (new BigN(freeBalance.value).lt(inputAmount)) {
      throw new Error(t('bg.handler.Extension.insufficientBalance'));
    }
  ...
  private qrSignSubstrate ({ address, data, networkKey }: RequestQrSignSubstrate): ResponseQrSignSubstrate {
    ...
    assert(pair, t('bg.handler.Extension.unableToFindAccount'));
  ...
  private async qrSignEVM ({ address, chainId, message, type }: RequestQrSignEvm): Promise<ResponseQrSignEvm> {
    ...
    if (!network) {
      throw new Error(t('bg.handler.Extension.cannotFindNetwork'));
    }

    const pair = keyring.getPair(address);

    if (!pair) {
      throw Error(t('bg.handler.Extension.unableToFindAccount'));
    }
    ...
    if (type === 'message') {
      ...
      if (!tx) {
        throw new Error(t('bg.handler.Extension.failedToDecodeQr'));
      }
  ...
  private async signingApprovePasswordV2 ({ id }: RequestSigningApprovePasswordV2): Promise<boolean> {
    ...
    if (!pair) {
      reject(new Error(t('bg.handler.Extension.unableToFindAccount')));
