...
import { openDefaultPopup, openPasskeyUnlockWindow } from '@subwallet/extension-base/services/request-service/handler/PopupHandler';
...
export default class KoniExtension {
  ...
  private async handleGetNftDetail (request: NftDetailRequest): Promise <NftItem | null> {
  ...
  private async makePsbtTransferAfterConfirmation (inputData_: RequestSubmitSignPsbtTransfer): Promise<SWTransactionResponse> {
    ...
    const errors = validateTransferRequest(transferTokenInfo, from, txOutput[0]?.address || '', value, false);
  ...
  private async getAddressAvailableBalanceByType ({ address, balanceType, extrinsicType, networkKey, token }: RequestAvailableBalanceByType): Promise<AmountData> {
    if (token && _MANTA_ZK_CHAIN_GROUP.includes(networkKey)) {
      const tokenInfo = this.#koniState.chainService.getAssetBySlug(token);
  ...
  private async subscribeAddressAvailableBalanceByType ({ address, balanceType, extrinsicType, networkKey, token }: RequestAvailableBalanceByType, id: string, port: chrome.runtime.Port): Promise<AmountData> {
    const cb = createSubscription<'pri(availableBalance.subscribeBalanceByType)'>(id, port);
  ...
  private async substrateNftSubmitTransaction (inputData: NftTransactionRequest): Promise<NftTransactionResponse> {
  ...
  private async accountsCreateMultisig (request: RequestAccountCreateMultisig): Promise<AccountMultisigError[]> {
  ...
  // Multisig handlers
  private async approvePendingTx (inputData: ApprovePendingTxRequest): Promise<SWTransactionResponse> {
  ...
  private async executePendingTx (inputData: ExecutePendingTxRequest): Promise<SWTransactionResponse> {
  ...
  private async cancelPendingTx (inputData: CancelPendingTxRequest): Promise<SWTransactionResponse> {
    const { address, callHash, chain, multisigMetadata, timepoint } = inputData;
  ...
  private async buildExtrinsicFromPayload (chain: string, payload: SignerPayloadJSON) {
    ...
    const { method, section } = substrateApi.api.registry.findMetaCall(call.callIndex);
  ...
  private async prepareMultisigSignRequest (request: PrepareMultisigSignRequest): Promise<PrepareMultisigSignResponse> {
    ...
    return {
      submittedCallData: submittedCallData as HexString,
  ...
  private async handleSubstrateProxyWrappedTx (
    ...
    if (isSignerProxiedAccount) {
      const restData = Object.fromEntries(
        Object.entries((originTransaction.data || {}) as Record<string, unknown>)
    ...
    if (isSignerProxiedAccount) {
      let callDataFinal: string | HexString = callData;
      let decodedCallDataFinal = decodedCallData;

      const pendingMultisigTxRequest = originTransaction.data as PendingMultisigTxRequest;
  ...
  private async keyringChangeMasterPassword (request: RequestChangeMasterPassword): Promise<ResponseChangeMasterPassword> {
    ...
    if (response.status) {
      ...
      await this.setPasskeyUnlockAction(false);
    }
  ...
  private keyringLock (): void {
    ...
    getPasskeyUnlockContext()
      .then((context) => this.setPasskeyUnlockAction(!!context))
      .catch(console.error);
  }

  private async passkeyUnlockReturnToPopup ({ windowId }: RequestPasskeyUnlockReturnToPopup): Promise<boolean> {
    // The popup override is cleared on unlock, but that runs detached, so re-apply it here to make
    // sure a popup is registered before asking the browser to open it.
    await this.setPasskeyUnlockAction(false);

    return await openDefaultPopup(windowId);
  }

  private async setPasskeyUnlockAction (enabled: boolean): Promise<void> {
    if (!chrome.action?.setPopup) {
      return;
    }

    await chrome.action.setPopup({ popup: enabled ? '' : 'index.html' }).catch(console.error);
  }
  ...
  private async signingApprovePasswordV2 ({ id }: RequestSigningApprovePasswordV2): Promise<boolean> {
    ...
    let signedTransaction: HexString | undefined;
    ...
    if (queued.signerAddress && isJsonPayload(payload)) {
      try {
        ...
        if (chain) {
          const { extrinsic } = await this.buildExtrinsicFromPayload(chain, payload);
  ...
  private async saveSubscanApiKey ({ apiKey }: RequestSaveSubscanApiKey) {
    return await this.#koniState.saveSubscanApiKey(apiKey);
  }

  private async getSubscanApiKey () {
  ...
  private async yieldGetEarningImpact (params: RequestEarningImpact) {
    const { slug } = params;
  ...
  public async handle<TMessageType extends MessageTypes> (id: string, type: TMessageType, request: RequestTypes[TMessageType], port: chrome.runtime.Port): Promise<ResponseType<TMessageType>> {
    ...
    switch (type) {
      ...
      case 'pri(nft.getFullList)':
        return await this.handleGetNftFullList(request as NftFullListRequest);
      case 'pri(nft.getNftdetail)':
        return await this.handleGetNftDetail(request as NftDetailRequest);
      ...
      case 'pri(accounts.create.multisig)':
        return await this.accountsCreateMultisig(request as RequestAccountCreateMultisig);
      ...
      case 'pri(availableBalance.getBalanceByType)':
        return this.getAddressAvailableBalanceByType(request as RequestAvailableBalanceByType);
      case 'pri(availableBalance.subscribeBalanceByType)':
        return this.subscribeAddressAvailableBalanceByType(request as RequestAvailableBalanceByType, id, port);
      ...
      case 'pri(substrateNft.submitTransaction)':
        return this.substrateNftSubmitTransaction(request as NftTransactionRequest);
      ...
      case 'pri(settings.saveSubscanApiKey)':
        return await this.saveSubscanApiKey(request as RequestSaveSubscanApiKey);
      ...
      case 'pri(keyring.passkeyUnlock.remove)':
        ...
        return;
      case 'pri(keyring.passkeyUnlock.openWindow)':
        return await openPasskeyUnlockWindow();
      case 'pri(keyring.passkeyUnlock.returnToPopup)':
        return await this.passkeyUnlockReturnToPopup(request as RequestPasskeyUnlockReturnToPopup);
      ...
      case 'pri(multisig.prepareSignRequest)':
        return await this.prepareMultisigSignRequest(request as PrepareMultisigSignRequest);
