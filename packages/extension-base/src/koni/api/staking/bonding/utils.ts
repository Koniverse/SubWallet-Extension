...
export const getMaxValidatorErrorMessage = (chainInfo: _ChainInfo, max: number): string => {
  let message = detectTranslate('bg.api.staking.bonding.utils.maxValidatorsSelection');
  const label = getValidatorLabel(chainInfo.slug);

  if (max > 1) {
    switch (label) {
      case 'dApp':
        message = detectTranslate('bg.api.staking.bonding.utils.maxDappsSelection');
        break;
      case 'Collator':
        message = detectTranslate('bg.api.staking.bonding.utils.maxCollatorsSelection');
        break;
      case 'Validator':
        message = detectTranslate('bg.api.staking.bonding.utils.maxValidatorsSelection');
        break;
    }
  } else {
    switch (label) {
      case 'dApp':
        message = detectTranslate('bg.api.staking.bonding.utils.maxOneDappSelection');
        break;
      case 'Collator':
        message = detectTranslate('bg.api.staking.bonding.utils.maxOneCollatorSelection');
        break;
      case 'Validator':
        message = detectTranslate('bg.api.staking.bonding.utils.maxOneValidatorSelection');
        break;
    }
  }
...
export const getExistUnstakeErrorMessage = (chain: string, type?: StakingType, isStakeMore?: boolean): string => {
  ...
  if (!isStakeMore) {
    switch (label) {
      case 'dApp':
        return t('bg.api.staking.bonding.utils.unstakeFromDappOnce');
      case 'Collator':
        return t('bg.api.staking.bonding.utils.unstakeFromCollatorOnce');

      case 'Validator': {
        if (type === StakingType.POOLED) {
          return t('bg.api.staking.bonding.utils.unstakeFromPoolOnce');
        }

        return t('bg.api.staking.bonding.utils.unstakeFromValidatorOnce');
      }
    }
  } else {
    switch (label) {
      case 'dApp':
        return t('bg.api.staking.bonding.utils.cannotStakeMoreOnUnstakingDapp');
      case 'Collator':
        return t('bg.api.staking.bonding.utils.cannotStakeMoreOnUnstakingCollator');

      case 'Validator': {
        if (type === StakingType.POOLED) {
          return t('bg.api.staking.bonding.utils.cannotStakeMoreOnUnstakingPool');
        }

        return t('bg.api.staking.bonding.utils.cannotStakeMoreOnUnstakingValidator');
      }
    }
  }
};
