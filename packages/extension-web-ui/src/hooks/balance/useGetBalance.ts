      ...
      timeout = setTimeout(() => {
        ...
        if (isChainActive) {
          ...
          const onCreateHandleError = (setter: Dispatch<SetStateAction<AmountData>>) => {
            return (e: Error) => {
              !cancel && setError(t('ui.USE_GET_BALANCE.hooks.balance.useGetBalance.unableToGetBalancePleaseReEnableTheNetwork'));
          ...
          !cancel && setIsLoading(false);
          !cancel && setError(t('ui.USE_GET_BALANCE.hooks.balance.useGetBalance.pleaseEnableOn', { tokenNames: tokenNames.join(', '), chain: chainInfo?.name }));
        }
      }, 600);
    }
