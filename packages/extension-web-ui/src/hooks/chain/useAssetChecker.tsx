...
export default function useAssetChecker () {
  ...
  useEffect(() => {
    if (enablingAsset && assetSettingMap[enablingAsset]?.visible) {
      ...
      const message = t('ui.USE_ASSET_CHECKER.hooks.chain.useAssetChecker.isTurnedOn', { replace: { name: assetInfo?.symbol } });
  ...
  return useCallback((assetSlug: string) => {
    ...
    if ((assetInfo && !assetSetting) || !assetSetting.visible) {
      const message = t('ui.USE_ASSET_CHECKER.hooks.chain.useAssetChecker.onIsNotReadyToUseDoYouWantToTurnItOn', {
        replace: {
      ...
      const _onEnabled = () => {
        updateAssetSetting({
          ...
          notify({ message: t('ui.USE_ASSET_CHECKER.hooks.chain.useAssetChecker.isTurningOn', { replace: { name: assetInfo?.symbol } }), duration: 1.5 });
        }).catch(console.error);
      };
      ...
      const btn = <Button
        ...
        size={'xs'}
      >
        {t('ui.USE_ASSET_CHECKER.hooks.chain.useAssetChecker.turnItOn')}
      </Button>;
      ...
      const message = t('ui.USE_ASSET_CHECKER.hooks.chain.useAssetChecker.chainIsDisconnected', { replace: { name: chainInfo?.name } });
