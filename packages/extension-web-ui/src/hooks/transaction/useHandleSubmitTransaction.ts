...
export const insufficientMessages = ['残高不足', 'Недостаточный баланс', 'bg.koni.handler.Extension.insufficientBalance'];
...
const useHandleSubmitTransaction = (setIgnoreWarnings?: (value: boolean) => void, handleDataForInsufficientAlert?: (estimateFee: AmountData) => Record<string, string>) => {
  ...
  const onSuccess = useCallback((rs: SWTransactionResponse) => {
    ...
    if (errors.length || warnings.length) {
      if (![t('ui.TRANSACTION.hook.transaction.useHandleSubmit.rejectedByUser'), 'bg.NETWORK.background.error.Provider.rejectedByUser'].includes(errors[0]?.message)) {
