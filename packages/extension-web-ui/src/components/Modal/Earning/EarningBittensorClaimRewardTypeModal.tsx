...
import { CHANGE_BITTENSOR_ROOT_CLAIM_TYPE_TRANSACTION, DEFAULT_CHANGE_BITTENSOR_ROOT_CLAIM_TYPE_PARAMS } from '@subwallet/extension-web-ui/constants';
import { ScreenContext } from '@subwallet/extension-web-ui/contexts/ScreenContext';
import { useHandleSubmitTransaction, usePreCheckAction } from '@subwallet/extension-web-ui/hooks';
import { changeBittensorRootClaimType } from '@subwallet/extension-web-ui/messaging';
...
import React, { Context, forwardRef, useCallback, useContext, useMemo, useState } from 'react';
...
const Component = (props: Props) => {
  ...
  const onCancel = useCallback(() => {
    setClaimTypeStorage(DEFAULT_CHANGE_BITTENSOR_ROOT_CLAIM_TYPE_PARAMS);
    inactiveModal(modalId);
  }, [inactiveModal, modalId, setClaimTypeStorage]);
  ...
  return (
    <SwModal
      className={`${className} bittensor-root-claim-type-modal ${isWebUI ? '-desktop' : ''}`}
      closeIcon={
        <Icon
          phosphorIcon={X}
          size='md'
...
const EarningBittensorClaimRewardTypeModal = styled(forwardRef(Wrapper))<Props>(({ theme: { token } }: Props) => {
  return {
    ...
    '@media (min-width: 768px)': {
      '&.bittensor-root-claim-type-modal': {
        width: '520px !important',

        '.ant-sw-modal-content.ant-sw-modal-content': {
          width: `${520 - token.lineWidth * 2}px !important`
        },

        '.ant-sw-modal-title, .ant-sw-sub-header-title': {
