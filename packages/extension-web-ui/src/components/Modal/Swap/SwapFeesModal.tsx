...
import { BN_TEN, BN_ZERO, SWAP_FEES_MODAL } from '@subwallet/extension-web-ui/constants';
...
const defaultTooltipMap: Record<SwapFeeType, (percentage?: number) => string> = {
  [SwapFeeType.PLATFORM_FEE]: () => detectTranslate('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.feePaidToThirdPartyProvidersToFacilitateTheSwapItIsNotPaidToSubwallet'),
  [SwapFeeType.NETWORK_FEE]: () => detectTranslate('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.feePaidToProcessYourTransactionOnTheBlockchainItIsNotPaidToSubwallet'),
  [SwapFeeType.WALLET_FEE]: (percentage?: number) => {
    if (!percentage) {
      return detectTranslate('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.feeChargedBySubwalletWhichIsAutomaticallyFactoredIntoThisQuote');
    } else {
      const message = detectTranslate('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.aFeeOfIsAutomaticallyFactoredIntoThisQuote');
...
const Component: FC<Props> = (props: Props) => {
  ...
  const feeItems = useMemo(() => {
    ...
    const feeConfigs = [
      { type: SwapFeeType.NETWORK_FEE, label: t('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.networkFee'), getTooltip: () => defaultTooltipMap[SwapFeeType.NETWORK_FEE]() },
      { type: SwapFeeType.PLATFORM_FEE, label: t('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.providerFee'), getTooltip: () => defaultTooltipMap[SwapFeeType.PLATFORM_FEE]() },
      { type: SwapFeeType.WALLET_FEE, label: t('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.subwalletFee'), getTooltip: (percentage?: number) => defaultTooltipMap[SwapFeeType.WALLET_FEE](percentage) }
    ];
    ...
    <BaseModal
      ...
      footer={(
        <Button
          ...
          onClick={onCancel}
        >
          {t('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.close')}
        </Button>
      )}
      id={modalId}
      onCancel={onCancel}
      title={t('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.swapFees')}
    >
      <MetaInfo
      ...
      >
        <div className={CN('__list-container')}>
          ...
          <MetaInfo.Number
            ...
            decimals={0}
            label={t('ui.SWAP_FEES_MODAL.components.Modal.Swap.SwapFeesModal.estimatedTotalFee')}
            prefix={(currencyData.isPrefix && currencyData.symbol) || ''}
            suffix={(!currencyData.isPrefix && currencyData.symbol) || ''}
            useNumberDisplay={true}
