...
import { BasicInputEvent, ChainSelector } from '@subwallet/extension-web-ui/components';
import { AUTHORIZE_TYPE_SUPPORTS_NETWORK_SWITCH, SWITCH_CURRENT_NETWORK_AUTHORIZE_MODAL } from '@subwallet/extension-web-ui/constants';
import { useGetCurrentAuth, useTranslation } from '@subwallet/extension-web-ui/hooks';
import { switchCurrentNetworkAuthorization } from '@subwallet/extension-web-ui/messaging';
  ...
  useEffect(() => {
    ...
    if (networkSelected && networkSelected !== authUrlInfo.currentNetworkMap[networkTypeSupported]) {
      ...
      switchCurrentNetworkAuthorization({ networkKey: networkSelected, authSwitchNetworkType: networkTypeSupported, url }).then(({ list }) => {
        showNotification({
          message: t('Switched network successfully')
        });
        onComplete(list);
