...
import { useGetAccountChainAddresses, useGetBitcoinAccounts, useHandleLedgerAccountWarning, useHandleLedgerGenericAccountWarning, useHandleTonAccountWarning, useIsPolkadotUnifiedChain, useNotification, useSelector, useTranslation } from '@subwallet/extension-web-ui/hooks';
import { AccountChainAddress, AccountInfoType, AccountTokenAddress, ThemeProps } from '@subwallet/extension-web-ui/types';
import { copyToClipboard, runPriorityWarningModalHandlers } from '@subwallet/extension-web-ui/utils';
...
function Component ({ accountProxy, className, isInModal, modalProps }: Props) {
  ...
  const onCopyAddress = useCallback((item: AccountChainAddress) => {
    return () => {
      ...
      const processFunction = () => {
        ...
        notify({
          message: t('ui.ACCOUNT_CHAIN_ADDRESS_LIST.components.AccountProxy.list.AccountChainAddressList.copiedToClipboard')
        });
      };
  ...
  return (
    <SwList.Section
      ...
      searchPlaceholder={t<string>('ui.ACCOUNT_CHAIN_ADDRESS_LIST.components.AccountProxy.list.AccountChainAddressList.enterNetworkNameOrAddress')}
    />
  );
}
