...
const Component: React.FC<Props> = (props: Props) => {
  ...
  const renderContent = () => {
    if (!value) {
      return placeholder || (t('ui.SELECT_VALIDATOR_INPUT.components.SelectValidatorInput.select') + ' ' + t(validatorLabel));
    }
    ...
    if (valueList.length > 1) {
      return t('ui.SELECT_VALIDATOR_INPUT.components.SelectValidatorInput.select') + ` ${valueList.length} ` + t(validatorLabel);
    }
