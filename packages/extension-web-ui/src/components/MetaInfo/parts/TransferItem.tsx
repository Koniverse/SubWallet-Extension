...
import { findAccountByAddress, toShort } from '@subwallet/extension-web-ui/utils';
    ...
    return (
      <>
        <AccountItem
          ...
          label={senderLabel || t('ui.TRANSFER_ITEM.components.MetaInfo.parts.TransferItem.sender')}
          name={senderName}
        />
        ...
        {!!originChain && !!destinationChain && originChain.slug === destinationChain.slug
          ? (
            <ChainItem
              ...
              label={t('ui.TRANSFER_ITEM.components.MetaInfo.parts.TransferItem.network')}
            />
          )
          : (
            <>
              {!!originChain && (
                <ChainItem
                  chain={originChain.slug}
                  label={t('ui.TRANSFER_ITEM.components.MetaInfo.parts.TransferItem.originChain')}
                />
              )}
              ...
              {!!destinationChain && (
                <ChainItem
                  ...
                  label={t('ui.TRANSFER_ITEM.components.MetaInfo.parts.TransferItem.destinationChain')}
                />
              )}
            </>
          )}
  ...
  return (
    <div className={CN(className, '__row -type-transfer')}>
      <div className={'__col __label-col'}>
        <div className={'__label'}>{senderLabel || t('ui.TRANSFER_ITEM.components.MetaInfo.parts.TransferItem.sender')}</div>
      ...
      <div className={'__col __value-col'}>
        <div className={'__label'}>{recipientLabel || t('ui.TRANSFER_ITEM.components.MetaInfo.parts.TransferItem.recipient')}</div>
