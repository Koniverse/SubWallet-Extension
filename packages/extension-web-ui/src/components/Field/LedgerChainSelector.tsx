...
function Component (props: Props, ref: ForwardedRef<InputRef>): React.ReactElement<Props> {
  ...
  const renderChainSelected = useCallback((item: LedgerItemType) => {
    if (loading) {
      return (
        <div className={'__loading-text'}>{t('ui.LEDGER_CHAIN_SELECTOR.components.Field.LedgerChainSelector.loading')}</div>
      );
    }
  ...
  return (
    <BaseSelectModal
      ...
      loading={loading}
      onSelect={onSelect}
      placeholder={placeholder || t('ui.LEDGER_CHAIN_SELECTOR.components.Field.LedgerChainSelector.selectChain')}
      ...
      searchPlaceholder={t<string>('ui.LEDGER_CHAIN_SELECTOR.components.Field.LedgerChainSelector.networkName')}
      selected={value || ''}
      statusHelp={statusHelp}
      title={title || label || placeholder || t('ui.LEDGER_CHAIN_SELECTOR.components.Field.LedgerChainSelector.selectNetwork')}
      tooltip={tooltip}
    />
  );
}
