...
const _SendFundOffRamp = ({ className = '', modalContent }: Props): React.ReactElement<Props> => {
  useSetCurrentPage('/transaction/off-ramp-send-fund');
  ...
  const destChain = defaultData.destChain;
  const transferAmount = defaultData.value;
  const from = defaultData.from;
  const chain = defaultData.chain;
  const asset = defaultData.asset;
...
const SendFundOffRamp = styled(_SendFundOffRamp)(({ theme }) => {
...
export default SendFundOffRamp;
