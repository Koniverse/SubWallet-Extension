  ...
  if (unstakingPeriod) {
    ...
    if (days < 1) {
      if (unstakingPeriod < 1) {
        ...
        return t('ui.EARNING_HANDLER.Popup.Transaction.helper.earning.earningHandler.minutes', { replace: { time: minutes } });
      }

      return t('ui.EARNING_HANDLER.Popup.Transaction.helper.earning.earningHandler.hours', { replace: { time: unstakingPeriod } });
    } else {
      return t('ui.EARNING_HANDLER.Popup.Transaction.helper.earning.earningHandler.days', { replace: { time: days } });
    }
  }
...
export function getWaitingTime (t: TFunction, currentTimestampMs: number, targetTimestampMs?: number, waitingTime?: number) {
  ...
  if (targetTimestampMs !== undefined) {
    ...
    if (waitingTime !== undefined) {
      ...
      return t('ui.EARNING_HANDLER.Popup.Transaction.helper.earning.earningHandler.automaticWithdrawal');
    }
  }

  if (remainingTimestampMs <= 0) {
    return t('ui.EARNING_HANDLER.Popup.Transaction.helper.earning.earningHandler.availableForWithdrawal');
  } else {
    ...
    return t('ui.EARNING_HANDLER.Popup.Transaction.helper.earning.earningHandler.withdrawableIn', { replace: { time: formattedWaitingTime } });
  }
}
