...
import { AccountAddressSelector, AddressInputNew, AddressInputRef, AmountInput, EarningValidatorSelector, NominationSelector, NumberDisplay } from '@subwallet/extension-web-ui/components';
import { useGetBalance, useGetChainPrefixBySlug, useTranslation, useWatchTransaction } from '@subwallet/extension-web-ui/hooks';
import { fetchPoolTarget } from '@subwallet/extension-web-ui/messaging';
import { store } from '@subwallet/extension-web-ui/stores';
import { AccountAddressItemType, Theme, ThemeProps, TransferParams } from '@subwallet/extension-web-ui/types';
...
const Component = ({ accountAddressItems, addressInputRef, addressInputRenderKey, amountInputRenderKey, assetInfo, chainValue, className, decimals, defaultData, destAssetInfo,
  destChainValue, disabledToAddressInput, form, fromValue, isNotShowAccountSelector, isReadonly, onBalanceReady, poolInfo, positionInfo, setIsTransferAll, setTargetLoading, validateRecipient }: Props) => {
  ...
  const validateAlphaAmount = useCallback((_rule: Rule, amount: string) => {
