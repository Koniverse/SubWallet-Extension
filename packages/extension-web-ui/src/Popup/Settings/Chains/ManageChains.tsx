...
import { AlertModal, FilterModal, Layout, NetworkEmptyList, NetworkToggleItem, OptionType, PageWrapper } from '@subwallet/extension-web-ui/components';
import { DataContext } from '@subwallet/extension-web-ui/contexts/DataContext';
import { ScreenContext } from '@subwallet/extension-web-ui/contexts/ScreenContext';
import { ChainInfoWithState, useAlert, useFilterModal, useNotification, useTranslation } from '@subwallet/extension-web-ui/hooks';
import useChainInfoWithStateAndStatus, { ChainInfoWithStateAndStatus } from '@subwallet/extension-web-ui/hooks/chain/useChainInfoWithStateAndStatus';
import { disableAllNetwork } from '@subwallet/extension-web-ui/messaging';
...
function Component ({ className = '' }: Props): React.ReactElement<Props> {
  ...
  const onDisableAll = useCallback(() => {
    if (!disablingAll && hasActiveChains) {
      openAlert({
        ...
        content: (
          <>
            <div>{t('ui.SETTINGS.screen.Setting.Chains.Manage.disableAllNetworkConfirmation')}</div>
            <div style={{ color: token.colorError, marginTop: token.marginXS }}>{t('ui.SETTINGS.screen.Setting.Chains.Manage.disableAllNetworkSubtitle')}</div>
          </>
        ),
        okButton: {
  ...
  }, [disablingAll, doDisableAll, hasActiveChains, openAlert, t, token.colorError, token.marginXS]);
