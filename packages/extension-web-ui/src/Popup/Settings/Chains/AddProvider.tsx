  ...
  const onSubmit = useCallback(() => {
    ...
    upsertChain(params)
      .then((result) => {
        ...
        if (result) {
          showNotification({
            message: t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.addedAProviderSuccessfully')
          });
          navigate(-1);
        } else {
          showNotification({
            message: t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.anErrorOccurredPleaseTryAgain')
          });
        }
      })
      .catch(() => {
  ...
  const handleErrorMessage = useCallback((errorCode: _CHAIN_VALIDATION_ERROR) => {
    switch (errorCode) {
      case _CHAIN_VALIDATION_ERROR.CONNECTION_FAILURE:
        return t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.cannotConnectToThisProvider');
      case _CHAIN_VALIDATION_ERROR.EXISTED_PROVIDER:
        return t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.thisProviderHasAlreadyBeenAdded');
      case _CHAIN_VALIDATION_ERROR.PROVIDER_NOT_SAME_CHAIN:
        return t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.thisProviderIsNotForThisNetwork');
      default:
        return t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.errorValidatingThisProvider');
    }
  }, [t]);
    ...
    return new Promise((resolve, reject) => {
      if (isUrl(provider)) {
        ...
        validateCustomChain(parsedProvider, chainInfo.slug)
          ...
          .catch(() => {
            setIsValidating(false);
            reject(new Error(t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.errorValidatingThisProvider')));
            setProviderValidation({ status: 'error', message: t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.errorValidatingThisProvider') });
          });
      } else {
        reject(new Error(t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.providerUrlIsNotValid')));
  ...
  return (
    ...
    >
      <Layout.Base
        leftFooterButton={{
          onClick: onCancel,
          children: t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.cancel')
        }}
        onBack={onBack}
        rightFooterButton={{
          block: true,
          ...
          loading: loading,
          onClick: onSubmit,
          children: t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.save')
        }}
        showBackButton={true}
        showSubHeader={true}
        ...
        title={t<string>('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.addNewProvider')}
      >
        <div className={'add_provider__container'}>
          <div className='description'>
            {t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.currentlySupportWssProviderForSubstrateNetworksAndHttpProviderForEvmNetwork')}
          </div>
          <Form
            form={form}
            ...
            <div className={'add_provider__attributes_container'}>
              ...
              >
                <Input
                  ...
                  placeholder={t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.providerUrl')}
                  prefix={<Icon
                    customSize={'24px'}
                    iconColor={token['gray-4']}
              ...
              <Row gutter={token.paddingSM}>
                <Col span={16}>
                  <Form.Item name={'name'}>
                    <Input
                      disabled={true}
                      placeholder={t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.networkName')}
                      prefix={(
                        <Icon
                          customSize={'24px'}
                          iconColor={token['gray-4']}
                      ...
                      tooltip={isWebUI ? t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.networkName') : undefined}
                ...
                <Col span={8}>
                  ...
                  >
                    <Input
                      disabled={true}
                      placeholder={t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.symbol')}
                      tooltip={isWebUI ? t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.symbol') : undefined}
                      tooltipPlacement='topLeft'
              ...
              <Form.Item name={'chainType'}>
                <Input
                  disabled={true}
                  placeholder={t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.networkType')}
                  tooltip={isWebUI ? t('ui.ADD_PROVIDER.Popup.Settings.Chains.AddProvider.networkType') : undefined}
                  tooltipPlacement='topLeft'
                  value={chainInfo.slug}
                />
              </Form.Item>
