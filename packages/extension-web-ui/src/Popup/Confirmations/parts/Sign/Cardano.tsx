...
import { completeConfirmationCardano } from '@subwallet/extension-web-ui/messaging';
import { CardanoSignatureSupportType, PhosphorIcon, ThemeProps } from '@subwallet/extension-web-ui/types';
  ...
  const onConfirm = useCallback(() => {
    ...
    if (txExpirationTime) {
      ...
      if (currentTime >= txExpirationTime) {
        notify({
          message: t('ui.DAPP.Confirmations.Sign.Cardano.transactionExpired'),
          type: 'error'
        });
        onCancel();
      }
  ...
  return (
    <div className={CN(className, 'confirmation-footer')}>
      ...
      }
      {
        isErrorTransaction
          ? <Button
            ...
            schema={'primary'}
          >
            {t('ui.DAPP.Confirmations.Sign.Cardano.iUnderstand')}
          </Button>
          : <Button
            disabled={loading}
            icon={(
            ...
            schema={'secondary'}
          >
            {t('ui.DAPP.Confirmations.Sign.Cardano.cancel')}
          </Button>
      }
      {!isErrorTransaction && <Button
        ...
        onClick={onConfirm}
      >
        {t('ui.DAPP.Confirmations.Sign.Cardano.approve')}
      </Button> }
    </div>
  );
};
