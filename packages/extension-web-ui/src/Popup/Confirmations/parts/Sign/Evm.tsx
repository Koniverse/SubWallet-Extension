...
import { useGetAccountByAddress, useGetChainInfoByChainId, useLedger, useNotification } from '@subwallet/extension-web-ui/hooks';
...
const Component: React.FC<Props> = (props: Props) => {
  ...
  }, [account?.address, address, chainId, evmWallet, isMessage, notify, onApproveSignature, onCancel, payload.payload]);
  ...
  const onConfirm = useCallback(() => {
    ...
    if (txExpirationTime) {
      ...
      if (currentTime >= txExpirationTime) {
        notify({
          message: t('ui.EVM.Popup.Confirmations.parts.Sign.Evm.transactionExpired'),
          type: 'error',
          duration: 8
        });
        onCancel();
  ...
  return (
    <div className={CN(className, 'confirmation-footer')}>
      ...
      }
      {
        isErrorTransaction
          ? <Button
            ...
            schema={'primary'}
          >
            {t('ui.EVM.Popup.Confirmations.parts.Sign.Evm.iUnderstand')}
          </Button>
          : <Button
            disabled={loading}
            icon={(
            ...
            schema={'secondary'}
          >
            {t('ui.EVM.Popup.Confirmations.parts.Sign.Evm.cancel')}
          </Button>
      }
      ...
      >
        {
          !isLedger
            ? t('ui.EVM.Popup.Confirmations.parts.Sign.Evm.approve')
            : !isLedgerConnected
              ? t('ui.EVM.Popup.Confirmations.parts.Sign.Evm.refresh')
              : t('ui.EVM.Popup.Confirmations.parts.Sign.Evm.approve')
        }
      </Button>}
      {
