...
- **The web-runner QC programme**: 20 sub-tasks — **16 done (132 pts), 1 closed unrun (8), 3 in progress on W38 (28) — nothing in `backlog`**. **The retest landed 09-16** (#5083): US-42.24.20 reads 54 fixed · 2 no-fix · 15 not fixed yet; **11 of the 15 are items the developer had already ticked on #2057**, one a P1 crash (`BUG-42.24.19-16`, item 59). Tester-owned; **do not move these stories between sprints**.
...
- **#2057 Mobile web-runner tracker** lives in [docs/notes/2026-09-04.md](docs/notes/2026-09-04.md) — tick-list, dev-side fix status only; the tester's verification is US-42.24.20 and is not mirrored. **The tracker spans several comments — three as of 09-14; read every one.**
- **US-12.23 is in its fourth window unchanged.** PR #5065 open and approved, #5064 closed by hand, dropped from 1.3.89. Re-checked 2026-09-14: head still **2026-08-27**, so its 16/16 QC is **18 days old** against a branch nobody builds. **W38 is the window to decide** — merge into `subwallet-dev` or close and set `backlog`. A fifth carry is the docs pretending a choice is pending.
- **US-10.21 / PR #5076 — reviewed 09-15, three findings fixed 09-16 (`67939c4315`), formal approval still pending.** Finding 1 was also a bug in the story: AC-6 said the screen shows the *domain*; the key is bound to the *origin*, and `https://www.app.com` / `https://app.com` / `http://app.com` derive three keys. Corrected. **Still missing**: the unit test for the background assert (AC-10), and a decision on `89e3c05e17` — a passkey-unlock setup modal with **no issue and no story** riding in the VRF PR (rule 9).
...
- **Three empty-body issues in under a month** — #5058, #5064, now **#5072** (sr25519 VRF signing for dApp key derivation, 2026-09-05) → [US-10.21](docs/sprints/stories/US-10.21-sr25519-vrf-signing-dapp-key-derivation.md), now `in-progress` and written from PR #5076's diff, EPIC-10 placement **provisional**. The first two were explained by their own PR, never by the issue; neither ever gained a body. Write stories from the diff, say so, and do not invent ACs.
