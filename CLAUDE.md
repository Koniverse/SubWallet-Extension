...
- **Pattern across all three W35 PRs: one review between them, and two of the three ship no test.** #5063 (P0, already hit a real transfer) 1 review / no test · #5061 0 reviews / 2 spec files · #5065 0 reviews / no test.
- **P0 — #5062.** The KAH-native `statemine-LOCAL-USDt` is offered an XCM route to Polkadot AH it cannot cross, so the tokens end up **locked on the destination chain** — locked, not destroyed; recovery is a separate manual exercise. Already hit a real transfer, and the route is still live. **The fix has two halves and only one is in this repo**: PR #5063 adds *validation logic* (`xcm/utils.ts` +162, approved 08-24), while the 4 chainlist data edits the issue prescribes have **no issue and no PR** in SubWallet-ChainList. Merging #5063 closes #5062 and leaves the bad route live in production data.
