...
| ~~NFR-11~~ | **RETIRED 2026-07-13 — memory is no longer a stated requirement of this product.** The row asserted a ≤72 MB budget via a "lightweight WsProvider" read path that was decided in 2022 and **never built** (AD-07). The only memory incident in 302 releases is the 0.2.2 crash that motivated it — nothing since; MV3's idle `sleep()` (which stops every chain API) is the de-facto control. **The number is retired, not renumbered — NFR-11 is never reused.** If a memory complaint appears: **measure first**, do not re-derive an AD from a guess (LESSONS §64). Decision: [CONTEXT D96](CONTEXT.md). | ⏸️ retired |
