...
> Synced from GitHub Projects board #2 ("SubWallet.App – Development"): issue #4552 is **Ready to Implement** there, so this story moves `backlog` → `ready` (sprint `sprint-2026-W28`). Only status/sprint changed; Goal, AC and reasoning below are untouched. The board is the live source for workflow state.
