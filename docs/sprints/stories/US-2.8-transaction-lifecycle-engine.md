...
- [ ] **AC-1** — **Given** any transaction type (transfer / swap / earn / XCM /
  ...
  flow.
- [ ] **AC-2** — **Given** the validate/preflight step, **When** the engine
...
- [ ] **AC-3** — **Given** a submitted transaction, **When** it is tracked,
...
- [ ] **AC-4** — **Given** validation fails or submission is rejected/errors,
...
- [ ] **TASK-2.8.1** — Shared status machine spanning transfer / swap / earn / XCM / governance (AC: 1)
- [ ] **TASK-2.8.2** — Validate/preflight: consult the fee engine and route signing through RequestService (AC: 2)
  - [ ] Build/submit extrinsics via ChainService APIs (US-2.2).
- [ ] **TASK-2.8.3** — Track status + record in-app submission, merge with per-chain indexer history (AC: 3)
- [ ] **TASK-2.8.4** — Typed failed status on validation/submission error; no false-positive history (AC: 4)
