...
many accounts, how quickly lists and screens render, how the WebApp and web-runner
web surfaces perform, and how trustworthy the dependency tree is. This epic ships
...
NFR-12, NFR-17, NFR-19, NFR-20, NFR-21) and absorbs the cross-cutting
...
the background lifecycle, the request layer, the UI render path, the web surfaces,
and the build toolchain. Left as footnotes inside feature epics, this program
...
collection screens; the WebApp and web-runner web-surface performance; and the
supply-chain hygiene (Yarn 3 lockfile integrity, npm-only registry) that keeps the
...
| 3 | **Memory & scale** | [US-20.3](../stories/US-20.3-read-path-memory-budget.md), [US-20.4](../stories/US-20.4-many-account-submit-performance.md) | Hold the ≤72 MB read-path budget regardless of chain count; stop the many-account submit / history-popup freeze |
...
| 5 | **Supply-chain hygiene** | [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md) | Yarn 3 lockfile integrity, npm-only registry, no untrusted/duplicate deps, an audit gate in CI |
...
> AD-08 (MV3 substrate), AD-24 (Services SDK) and AD-25 (cache/CDN proxy) are
> *referenced* here for the runtime EPIC-20's invariants ride on; their primary
> implementation lives in [EPIC-1](EPIC-1.md) (AD-08) and [EPIC-2](EPIC-2.md)
> (AD-24, AD-25). EPIC-20 enforces the lifecycle/request/memory contracts on top
> of them rather than re-deriving them. NFR-19 (supply-chain hygiene,
> [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md))
> rides the Yarn 3 monorepo (AD-05) and bundle-splitting (AD-06) owned by
> [EPIC-1](EPIC-1.md); it anchors no new AD of its own.
...
| [NFR-19](../../PRD.md#non-functional-requirements) | Dependency auditability: Yarn 3 lockfile, npm-only registry | [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md) |
...
| [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md) | Dependency auditability & supply-chain hygiene | Keep the dependency tree auditable: Yarn 3 lockfile integrity, npm-only registry, audit gate in CI | 📋 backlog | — |
...
| [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md) | Yarn 3 `yarn.lock` + CI supply-chain gate (immutable install, registry assertion, dedupe/audit) | — |
...
- **Supply-chain lockfile hygiene ([NFR-19](../../PRD.md#non-functional-requirements)):** the Yarn 3 `yarn.lock` is committed and integrity-checked, all core-wallet dependencies resolve from the public npm registry (no private/untrusted registries), and CI fails on a dirty/out-of-sync lockfile or a flagged advisory. Enforced by [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md).
...
| **Lockfile / audit gate** | [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md) | A CI step asserting `yarn.lock` is clean, registry is npm-only, and the advisory audit passes |
...
| **Lockfile integrity** | `yarn.lock` committed and clean in CI; npm-only registry; audit gate green | [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md) | A drifted lockfile or private-registry dep silently changes the shipped bytes (NFR-19) |
...
- [ ] CI fails on a dirty/out-of-sync `yarn.lock`, a non-npm registry dependency, or a flagged advisory — [US-20.7](../stories/US-20.7-dependency-auditability-and-supply-chain-hygiene.md)
...
- [Source: PRD §Non-Functional Requirements NFR-8, NFR-11, NFR-12, NFR-17, NFR-19, NFR-20, NFR-21](../../PRD.md#non-functional-requirements)
