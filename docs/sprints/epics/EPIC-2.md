...
| ~~**WsProvider read-path / memory contract**~~ **retired** | [US-2.2](../stories/US-2.2-chainservice-live-api-per-chain.md), [US-2.5](../stories/US-2.5-balance-detection-and-aggregation-engine.md), [US-2.6](../stories/US-2.6-fee-engine.md) | AD-07's claimed fixture never existed. There is no standing memory contract; measure first if a memory complaint creates new scope ([CONTEXT D96](../../CONTEXT.md)). |
...
|---|---|---|---|
| ~~**Multi-chain read-path RAM**~~ **retired** | ~~WsProvider-only envelope (~72 MB regardless of chain count)~~ — no read-path memory budget is stated ([CONTEXT D96](../../CONTEXT.md)) | [US-2.2](../stories/US-2.2-chainservice-live-api-per-chain.md), [US-2.5](../stories/US-2.5-balance-detection-and-aggregation-engine.md) | The 2022 ApiPromise/WsProvider comparison was never implemented or re-measured under MV3. |
...
- [ ] ChainService maintains a live API object per network across 200+ chains with connect/disconnect/retry — [US-2.2](../stories/US-2.2-chainservice-live-api-per-chain.md)
