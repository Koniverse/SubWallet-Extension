...
- **Web surfaces are hardened separately from the extension ([NFR-25](../../PRD.md#non-functional-requirements)):** the web app and web-runner enforce a Content-Security-Policy, and every externally-controlled link carries `noopener`/`noreferrer` (reverse tabnabbing). **The extension's MV3 CSP ([NFR-8](../../PRD.md#non-functional-requirements)) does not cover them** — they are separately built and separately exposed, which is exactly why the Verichains audit found them (#4959). Enforced by [US-5.10](../stories/US-5.10-verichains-audit-remediation-hardening.md). NFR-25 was a **PRD gap until 2026-07-13** ([D98](../../CONTEXT.md)).
