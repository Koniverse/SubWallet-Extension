...
| [US-8.20](../stories/US-8.20-open-transaction-improvements.md) | Open transaction improvements | Address validation round 2, more history types, the fee service nobody built | 📋 backlog | — |
...
- **A wrong number is worse than no number ([NFR-22](../../PRD.md#non-functional-requirements)):** every fee, gas estimate and amount the user is shown either matches its source of truth or is rendered as **unavailable** — never as a confident wrong figure. This is the user-facing half of the BigInt invariant above: the arithmetic being exact does not help if the input was guessed. Enforced by [US-8.12](../stories/US-8.12-fee-bigint-and-gas-estimation-hardening.md).
- **Degrade, never blank ([NFR-24](../../PRD.md#non-functional-requirements)):** a malformed, unexpected or undecodable payload — from a dApp, a chain, or an indexer — degrades to an explicit error the user can act on, never a blank screen or a crashed popup. This binds hardest on the **confirmation screen**, where the user is being asked to sign something they cannot see. Enforced by [US-8.13](../stories/US-8.13-payload-decode-error-handling.md). NFR-22 and NFR-24 were both **PRD gaps until 2026-07-13** ([D98](../../CONTEXT.md)) — the invariants were real and already being defended; only the requirements were missing.
